...

{
  ...
  public:
      ...
      for (int ebNE = 0; ebNE < nExteriorElementBoundaries_global; ebNE++)
	{
	  ...
	  for  (int kb=0;kb<nQuadraturePoints_elementBoundary;kb++)
	      ...
	      for (int j=0;j<nDOF_trial_element;j++)
		{
		  ...
		  //anb_seepage_flux +=flux_ext*dS;
		  //std::cout<<"Seepage Flux" <<anb_seepage_flux<<std::endl;
		  
		  //if (isSeepageFace)
		  //{
		  //  anb_seepage_flux +=flux_ext*dS;
		//	std::cout<<"Seepage face at last!"<<std::endl;
		  //}
		 // else {
		//	std::cout<<"Flux ext" <<flux_ext<<"Weightage" <<dS <<std::endl;
		 // }
      ...
      for (int i=0; i<numDOFs; i++)
	{
	  ...
	  //////////////////////////////////////////////////////////////////////
	  ////////ADDING ANTIDIFFUSIVE TERM IN STABILIZATION TYPE 2/////////////
	  ///////////////////////////////////////////////////////////////////////

	  // first step is computing limiters 
	  ...
	  for (int offset=csrRowIndeces_DofLoops.data()[i]; offset<csrRowIndeces_DofLoops.data()[i+1]; offset++)
	    {
	      ...
	      double I_plus_ML_minus_MC = (i==j ? 1. : 0.)*(1. + mi) - MassMatrix.data()[ij];
	      //sdot[i] +=  I_plus_ML_minus_MC*(solH.data()[j]-soln.data()[j])/mi;
		  sdot[i] +=  I_plus_ML_minus_MC*(sn.data()[j]-sLow.data()[j])/mi;
		  FluxCorrectionMatrix[ij] = (LUMPED_MASS_MATRIX == 1 ? 0. : 1.)*dt*MassMatrix.data()[ij]*(sdoti - sdotj) + dt_times_fH_minus_fL.data()[ij];
      ...
      for (int i=0; i<numDOFs; i++)
	{
	  ...
	  //std::cout<<"Limiter "<< ith_Limiter_times_FluxCorrectionMatrix  <<std::endl;

	  //sLow.data()[i] = mn + dt*uDotLow.data()[i]*bc_mask.data()[i];//cek should introduce mn,mnp1 or somethign clearer
	  globalResidual.data()[i] = (mi*(m - mn)/dt - ith_flux_term)*bc_mask.data()[i];//+ ith_Limiter_times_FluxCorrectionMatrix;//cek should introduce mn,mnp1 or somethign clearer
