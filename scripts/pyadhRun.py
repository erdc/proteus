...
def testElementBoundaryTags(m):
    for ebNE in range(m.nExteriorElementBoundaries_global):
        ebN = m.exteriorElementBoundariesArray[ebNE]
        #assert m.elementBoundaryMaterialTypes[ebN] == 1, "failed exterior ebN= %d " % ebN
        assert m.elementBoundaryMaterialTypes[ebN] >= 1, "failed exterior ebN= %d " % ebN
    for ebNI in range(m.nInteriorElementBoundaries_global):
        ebN = m.interiorElementBoundariesArray[ebNI]
        if m.elementBoundaryMaterialTypes[ebN] != 0:
            #mwf debug
            import pdb
            pdb.set_trace()
...
#             n.nLevels += 1
            ...
            if p.polyfile != None:
                #mwftodo decide if want to force .face file to exist or not
                if not (os.path.exists(p.polyfile+".ele") and
                        os.path.exists(p.polyfile+".node") and
                        os.path.exists(p.polyfile+".face")):
                    #mwf debug
                    #import pdb
                    #pdb.set_trace()
                    #need to force -f in options to get full set of element boundary faces
                    #if n.triangleOptions.find('f') < 0:
                    #    n.triangleOptions += 'f'
...
#         cPickle.dump(mlMesh,mlMeshFile,protocol=cPickle.HIGHEST_PROTOCOL)
    ...
    while ((error > tol) and
        ...
        mlTransport = Transport.MultilevelTransport(
            ...
            n.numericalFluxType,
            n.timeIntegration,
            massLumping = n.massLumping,
            reactionLumping = n.reactionLumping)
    ...
    #         EWtol=False)
        ...
        if simFlags != None:
            ...
            simOutput = SimTools.SimulationProcessor(pFile=p,nFile=n)
        errorEstimator = ErrorEstimators.HierarchicalMeshEstimator(mlTransport)

        simOutput.preprocess(mlTransport,tn)
        ...
        #end while T
        simOutput.postprocess(mlTransport,tn)
    #end adapt mesh
