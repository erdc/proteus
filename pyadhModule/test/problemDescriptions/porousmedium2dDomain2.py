...
def pome2D( nx,
    ...
    a=int(points_on_boundary*3.0*radius)
    b=int(points_on_boundary*2.0*radius)

    DA=3.0*radius/(float(a))
    DB=3.0*radius/(float(b))

    for k in range(a+1):
        domain_vertices.append((k*DA, 0.0))
        domain_vertices.append((k*DA, 1.0))
        domain_vertices.append((1.0-k*DA,0.0))
        domain_vertices.append((1.0-k*DA,1.0))
    for k in range(b+1):
        for i in range(nx-2):
            domain_vertices.append((5.0*radius+i*2.0*radius+k*DB,0.0))
            domain_vertices.append((5.0*radius+i*2.0*radius+k*DB,1.0))
...
##     radius = 0.25*min(dx,dy)
    #radius = 1.0/(4.0*float(nx))
    grain_centers  = []
    half_grain_centers_bot=[]
    half_grain_centers_top=[]
...
##             grain_centers.append((i*dy+0.5*dy,j*dx+0.5*dx))
    ...
    for i in range(nx-1):
        half_grain_centers_top.append((4.0*radius+float(i)*4.0*radius,1.0))
        half_grain_centers_bot.append((4.0*radius+float(i)*4.0*radius, 0.0))
    ...
    nvertices = len(domain_vertices) + len(grain_centers)*points_on_grain+len(half_grain_centers_top)*(points_on_grain/2)*2
