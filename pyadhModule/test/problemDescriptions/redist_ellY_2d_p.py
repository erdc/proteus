...
# \ref redist_ellY_2d_p.py "Hamilton Jacobi LS redistancing (circle)"
# 

##\ingroup test
# \file redist_ellY_2d_p.py
# @{
...
# \f}
#
# In this case, the domain is an "ell" given by the union of the segments 
#    x=0.5, y in [0.5,1.0]
# and
#    y=0.5, x in [0.5,1.0]
# and the solution is the distance to the y axis
...
class EllYSquared:
    def __init__(self,topY=1.0,cornerX=0.5,cornerY=0.5):
        self.cornerX = cornerX
        self.cornerY = cornerY
        self.topY    = topY
    def uOfXT(self,x,t):
        return (x[1]-self.topY)**2
class EllY:
    """ 
    don't know if this is really exact solution or not
    """
    def __init__(self,topY=1.0,cornerX=0.5,cornerY=0.5):
        self.cornerX = cornerX
        self.cornerY = cornerY
    ...
    def uOfXT(self,x,t):
        ...
        return sqrt((x[1]-self.topY)**2)
x0 = 0.50
y0 = 0.50
y1 = 1.0
analyticalSolution = {0:EllY(y1,x0,y0)}
...
coefficients = RedistanceLevelSet(epsFact=1.5,u0=EllY(y1,x0,y0))
...
initialConditions  = {0:EllYSquared(y1,x0,y0)}
