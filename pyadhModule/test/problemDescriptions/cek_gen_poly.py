...
def genPoly(fileprefix,
        ...
        for j in range(ny):
            for k in range(nx):
                ...
                if k > 0:
                    ...
                    nN+=1
                if i==0:
                    pointFacets[0].append(south_poles[(i,j,k)])
                if i==nz-1:
                    pointFacets[5].append(north_poles[(i,j,k)])
                if j==0:
                    pointFacets[2].append(front_poles[(i,j,k)])
                if j==ny-1:
                    pointFacets[3].append(back_poles[(i,j,k)])
                if k==0:
                    pointFacets[1].append(left_poles[(i,j,k)])
                if k==nx-1:
                    pointFacets[4].append(right_poles[(i,j,k)])
    ...
    for fN,f in enumerate(facets[:6]):
        #print "Face"
        poly.write('%d %d %d \n' % (1+len(pointFacets[fN]),0,fN+1)) #1 polygon, 0 holes, 1 on boundary, check consistent with boundaryTags
        poly.write('%d %d %d %d %d\n' % (4,f[0]+1,f[1]+1,f[2]+1,f[3]+1)) #facet number and corners  of facet
        #for v in f:
        #    print vertices[v]
        for pfN in pointFacets[fN]:
            poly.write('%d %d \n' % (1,pfN+1)) #facet number and corners  of facet
            #print vertices[pfN]
        #no holes in facet
    for fN,f in enumerate(facets[6:]):
