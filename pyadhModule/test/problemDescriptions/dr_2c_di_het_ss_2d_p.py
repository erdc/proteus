...
#\ref dr_2c_di_het_ss_2d_p.py "Linear diffusion-reaction, two-component, diagonal system, at equilibrium (2D)"
#

##\ingroup test
#\file dr_2c_di_het_ss_2d_p.py
#
#\brief linear diffusion-reaction system with two-components at equilibrium in 2D. includes
#  constant 'gravity-like' term to test models for linear flow 
#\todo finish dr_2c_di_het_ss_2d_p.py impl,doc
nd = 2
...
# # # # # # # # # # # # # # # # # # # # # # # # # 
#u = 1- x_{i}, a = I, b = [0,-1], c = 1.0 f = 1- x_i 
#dir bc's everywhere
ax = 0
def a0(x):
    return Ident
...
# # # # # # # # # # # # # # # # # # # # # # # # # 
#u = \sum x_{i}, a = I, b = [0,-1], c = 1.0 f = \sum x_i 
#dir bc's everywhere
def a1(x):
    return Ident
def b1(x):
...
# # # # # # # # # # # # # # # # # # # # # # # # # 
#u = sin^2(2\pi x) + cos^2(2\pi y) + x+y + 5.0, a = I, b = [0,-1], c = 1.0
#f = -8\pi^2cos^2(2\pi x) + 8\pi^2cos^2(2\pi y)
#    +8\pi^2sin^2(2\pi x) - 8\pi^2sin^2(2\pi y) + sin^2(2\pi x) + cos^2(2\pi y) + x+y + 5.0
def a3(x):
    return Ident
def b3(x):
    return gravityU
def c3(x):
    return 1.0
def f3(x):
    return 8.0*pi**2*(-cos(2.*pi*x[0])**2 + cos(2.0*pi*x[1])**2 + sin(2.0*pi*x[0])**2 - sin(2.0*pi*x[1])**2) + sin(2.0*pi*x[0])**2 + cos(2.0*pi*x[1])**2 + x[0]+x[1]+5.0
...
#   + 4\pi*cos(2\pi*y)*sin(2\pi*y) + x+y
#dirichlet bc's everywhere
...
def c4(x):
    return 1.0
def f4(x):
    return 4.0*pi*(-cos(2.0*pi*x[0])*sin(2.0*pi*x[0]) + cos(2.0*pi*x[1])*sin(2.0*pi*x[1])) + 4.0*pi*cos(2.0*pi*x[1])*sin(2.0*pi*x[1]) + x[0]+x[1]
...
#u = x + y, a_00 = sin^2(2\pi x) + 5, a_11 = cos^2(2\pi y) + 5.0
#f = 4\pi*(-cos(2\pi*x)sin(2\pi*x) + cos(2\pi*y)sin(2\pi*y))
#   + 4\pi*y*cos(2\pi*y)*sin(2\pi*y) - cos^2(2\pi y) + x+y
#dirichlet bc's everywhere
...
def c4a(x):
    return 1.0
def f4a(x):
    return 4.0*pi*(-cos(2.0*pi*x[0])*sin(2.0*pi*x[0]) + cos(2.0*pi*x[1])*sin(2.0*pi*x[1])) + 4.0*pi*x[1]*cos(2.0*pi*x[1])*sin(2.0*pi*x[1]) - cos(2.0*pi*x[1])**2 -5.0 + (x[0]+x[1])*1.0
...
#
# # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # 
#u = x + y, a_00 = sin^2(2\pi x) + 5, a_11 = cos^2(2\pi y) + 5.0, c = x
#f = 4\pi*(-cos(2\pi*x)sin(2\pi*x) + cos(2\pi*y)sin(2\pi*y))
#   + 4\pi*y*cos(2\pi*y)*sin(2\pi*y) - cos^2(2\pi y) + (x+y)*x
#dirichlet bc's everywhere
...
def c4b(x):
    return x[0]
def f4b(x):
    return 4.0*pi*(-cos(2.0*pi*x[0])*sin(2.0*pi*x[0]) + cos(2.0*pi*x[1])*sin(2.0*pi*x[1])) + 4.0*pi*x[1]*cos(2.0*pi*x[1])*sin(2.0*pi*x[1]) - cos(2.0*pi*x[1])**2 -5.0 + (x[0]+x[1])*x[0]
...
#
# # # # # # # # # # # # # # # # # # # # # # # # # 
#u = x*x + y*y, a_00 = x + 5, a_11 = y + 5.0, b = [0,-y], c = (x+1)
#f = -2*x -2*(5+x) -2*y-2*(5+y) -2*y - 5 + (x+1)*(x*x+y*y)
#dirichlet bc's everywhere
def a5(x):
...
def c5(x):
    return x[0]+1.0
def f5(x):
    return -2.0*x[0] -2*(5.+x[0]) -2.*x[1]-2.*(5.+x[1]) -2.0*x[1]-5.0 +(x[0]+1.)*(x[0]**2+x[1]**2)
...
#   -3*(y*y - 5) + x*(sin^2(2*pi*x) + cos^2(2*pi*y) + x + y + 5.)
# # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # 
#dirichlet bc's everywhere
...
def f6(x):
    return -2.*x[0]*(1. + 4.*pi*cos(2.*pi*x[0])*sin(2.*pi*x[0])) - \
           ...
           3.0*x[1]**2 - 5.0 + x[0]*(sin(2.0*pi*x[0])**2 + cos(2.*pi*x[1])**2 + x[0]+x[1]+5.)
