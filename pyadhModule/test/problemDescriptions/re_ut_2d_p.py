...
heterogeneityBlocks[zone]['Xm']  = [0.0,7.5,0.0]; heterogeneityBlocks[zone]['Xp'] = [L[0],L[1],1.0]
heterogeneityBlocks[zone]['type']= 0
#0.0 <= z <=7.5
zone = 1
heterogeneityBlocks[zone] = {};
heterogeneityBlocks[zone]['Xm']  = [0.0,0.0,0.0]; heterogeneityBlocks[zone]['Xp'] = [L[0],7.5,1.0]
...
eps = 1.0e-8
def getDBC_2D_Richards(x,flag):
    if x[1] >= L[1] - eps:
        return lambda x,t: pondingHead
    elif x[1] <= eps:
        return lambda x,t: bottomHead

dirichletConditions = {0:getDBC_2D_Richards}
