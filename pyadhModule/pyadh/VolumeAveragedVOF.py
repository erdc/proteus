...
import cVolumeAveragedVOF,cVolumeAveragedVOF2D,cVolumeAveragedVOFQ

class OneLevelVolumeAveragedVOF(OneLevelTransport):
    nCalls=0
    def __init__(self,
                 uDict,
                 phiDict,
...
#         self.numericalFlux.ebqe[('u',0)].flat[:]=1.0
        ...
        if isinstance(self.u[0].femSpace,C0_AffineLinearOnSimplexWithNodalBasis):
            cResidual = cVolumeAveragedVOF.calculateResidual
            if self.nSpace_global == 2:
                cResidual = cVolumeAveragedVOF2D.calculateResidual
        elif isinstance(self.u[0].femSpace,C0_AffineQuadraticOnSimplexWithNodalBasis):
            cResidual = cVolumeAveragedVOFQ.calculateResidual
            if self.nSpace_global == 2:
                cResidual = cVolumeAveragedVOF2DQ.calculateResidual

        #cVolumeAveragedVOF.calculateResidual(self.mesh.nElements_global,
        cResidual(self.coefficients.epsFact,
                  self.mesh.nElements_global,
	...
	cfemIntegrals.zeroJacobian_CSR(self.nNonzerosInJacobian,
        ...
        #mwf debug
        #pdb.set_trace()
        cJacobian = cVolumeAveragedVOF.calculateJacobian
        if self.nSpace_global == 2:
            cJacobian = cVolumeAveragedVOF2D.calculateJacobian
        elif self.nSpace_global == 1:
            cJacobian = cVolumeAveragedVOF1D.calculateJacobian

        #cVolumeAveragedVOF.calculateJacobian(self.mesh.nElements_global,
