...
cd petsc
./configure --prefix=${PROTEUS_PREFIX} --download-hdf5 --download-tetgen --download-triangle  --download-petsc4py --download-mpi4py --download-superlu --download-parmetis --download-metis --download-superlu_dist --download-zlib --download-hypre --download-zoltan --download-eigen --download-mpich --download-cmake
make PETSC_DIR=${PWD} all
make install
cd ..
info "Installing post-petsc python packages"
export MPICXX=${PROTEUS_PREFIX}/bin/mpicxx
export MPICC=${PROTEUS_PREFIX}/bin/mpicc
