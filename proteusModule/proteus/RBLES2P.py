...
import cRBLES2P #,cRBLES2P2D,cRBLES2PQ#,cRBLES2P2DQ

class OneLevelRBLES2P(OneLevelTransport):
    nCalls=0
    def __init__(self,
                 uDict,
                 phiDict,
    ...
    def getResidual(self,u,r):
        ...
        """
        if isinstance(self.u[0].femSpace,C0_AffineLinearOnSimplexWithNodalBasis):
            cResidual = cRBLES2P.calculateResidual
            if self.nSpace_global == 2:
                cResidual = cRBLES2P2D.calculateResidual
        elif isinstance(self.u[0].femSpace,C0_AffineQuadraticOnSimplexWithNodalBasis):
            cResidual = cRBLES2PQ.calculateResidual
            #if self.nSpace_global == 2:
            #    cResidual = cRBLES2P2DQ.calculateResidual
...
#	sys.stdout.write('isDOFBoundary 2')
#        print          self.numericalFlux.isDOFBoundary[2]
...
#        print          self.numericalFlux.isDOFBoundary[3]
        ...
        cResidual(self.mesh.nElements_global,
                  ...
                  self.offset[0],self.offset[1],self.offset[2],self.offset[3],self.stride[0],self.stride[1],self.stride[2],self.stride[3],r,
    ...
    def getJacobian(self,jacobian):
        ...
        import numpy
        if isinstance(self.u[0].femSpace,C0_AffineLinearOnSimplexWithNodalBasis):
            cJacobian = cRBLES2P.calculateJacobian
            if self.nSpace_global == 2:
                cJacobian = cRBLES2P2D.calculateJacobian
        elif isinstance(self.u[0].femSpace,C0_AffineQuadraticOnSimplexWithNodalBasis):
            cJacobian = cRBLES2PQ.calculateJacobian
            #if self.nSpace_global == 2:
            #    cJacobian = cRBLES2P2DQ.calculateJacobian
    ...
    def calculateAuxiliaryQuantitiesAfterStep(self):
        if isinstance(self.u[0].femSpace,C0_AffineLinearOnSimplexWithNodalBasis):
            cva = cRBLES2P.calculateVelocityAverage
            if self.nSpace_global == 2:
                cva = cRBLES2P2D.calculateVelocityAverage
        elif isinstance(self.u[0].femSpace,C0_AffineQuadraticOnSimplexWithNodalBasis):
            cva = cRBLES2PQ.calculateVelocityAverage
            #if self.nSpace_global == 2:
            #    cva = cRBLES2P2DQ.calculateVelocityAverage
