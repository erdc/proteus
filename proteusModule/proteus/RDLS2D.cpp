...
//extern "C" void calculateResidual_RDLS(int nElements_global,
extern "C" void RDLS_RES (int nElements_global,
			  double alphaBDF,
			  ...
			  double* ebqe_u)
{
  ...
  //lag_subgridError=2;
  //using namespace RDLS;
  using namespace RDLS_NAME; 
  //
...
// 	    {
// 	      std::cout<<"RDLS res eN= "<<eN<<" u= "<<u<<" m= "<<m<<" alphaBDF= "<<alphaBDF<<" m_t= "<<m_t<<" dm_t= "<<dm_t<<" r= "<<r<<" dH[0]= "<< dH[0]<<" dH_sge[0]= "<< dH_sge[0]<<" grad_u[0]="<<grad_u[0]
      ...
      if (freezeLevelSet)
	{
	  ...
	  for (int j = 0; j < nDOF_trial_element; j++)
	      ...
	      if (weakDirichletConditionFlags[J] == 1)
		{
		  ...
		  //mwf debug
		  //std::cout<<"RDLS freeze eN= "<<eN<<" j= "<<j<<" J= "<<J<<" u_dof= "<<u_dof[J]<<" u_weak_bc= "<<u_weak_internal_bc_dofs[J]<<std::endl;
		}
	    }//j
	}//freeze
...
// 	  if (kb >= 0)
// 	    {
// 	      std::cout<<"RDLS res ebNE= "<<ebNE<<" ebN= "<<ebN<<" kb= "<<kb<<" ebNE_kb= "<<ebNE_kb <<" u_ext= "<<u_ext<<" m_ext= "<<m_ext
// 		       <<std::endl;
// 	    }
	  for (int i=0;i<nDOF_test_element;i++)
	    {
...
//for now assumes that using time integration
//and so lags stabilization and subgrid error
//extern "C" void calculateJacobian_RDLS(int nElements_global,
extern "C" void RDLS_JAC (int nElements_global,
			  double alphaBDF,
			  double epsFact_redist,
			  int freezeLevelSet,
			  int useTimeIntegration,
...
// 	    {
// 	      std::cout<<"RDLS jac eN= "<<eN<<" u= "<<u<<" m= "<<m<<" alphaBDF= "<<alphaBDF<<" m_t= "<<m_t<<" dm_t= "<<dm_t
      ...
      if (freezeLevelSet)
	{
	  ...
	  for (int j = 0; j < nDOF_trial_element; j++)
	      ...
	      if (weakDirichletConditionFlags[J] == 1)
		{
		  ...
		  //mwf debug
		  //std::cout<<"RDLS Jac freeze eN= "<<eN<<" j= "<<j<<" J= "<<J<<std::endl;
		}
	    }
	}
      for (int i=0;i<nDOF_test_element;i++)
