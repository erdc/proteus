...
inline
double HessFromDOF_c(const double& dof,const double& Hess_v_IJ)
{
  return dof*Hess_v_IJ;
}
...
inline
double Diffusion_strong_c(const double& a,
			  const double Hess_u[nSpace2])
{
  double tmp=0.0;
  for(int I=0;I<nSpace;I++)
    tmp -= a*Hess_u[I*nSpace+I];
  return tmp;
}

inline
double DiffusionJacobian_strong_c(const double& a,
				  const double Hess_v[nSpace2])
{
  double tmp=0.0;
  for(int I=0;I<nSpace;I++)
    tmp -= a*Hess_v[I*nSpace+I];
  return tmp;
}

inline
double Diffusion_adjoint_c(const double& a,
			   const double Hess_w_dV[nSpace2])
{
  double tmp=0.0;
  for(int I=0;I<nSpace;I++)
    tmp -= a*Hess_w_dV[I+I*nSpace];
  return tmp;
}
...
extern "C"
{
  void calculateResidual_RANS2P2DQ(int nElements_global,
				 double dt,
				 double eps_rho,
				 double eps_mu,
				 double sigma,
				 ...
				 double* q_mom_v_acc,
				 double* q_mass_adv,
				 double* q_mom_u_acc_last, double* q_mom_v_acc_last,
				 ...
				 int offset_p, int offset_u, int offset_v, int stride_p, int stride_u, int stride_v,  double* globalResidual,
...
void calculateJacobian_RANS2P2DQ(int nElements_global,
					    double alphaBDF,
		       double epsFact_rho,
		       double epsFact_mu,
		       double sigma,
  ...
  void calculateVelocityAverage_RANS2P2DQ(int nExteriorElementBoundaries_global,
					  int* exteriorElementBoundariesArray,
