#ifndef LADR1DELLAM_H
#define LADR1DELLAM_H
#include <cmath>
#include <iostream>
...
namespace LADR1Dellam
{
inline
void evaluateTestFunctionsOnElement(const double x[3], const double x0[3], const double x1[3], double w[nDOF_test_element])
{
			    ...
			    double a[nSpace])
{
  ...
  normq = sqrt(normq); 
  for (int I = 0; I < nSpace; I++)
    a[I*nSpace + I]  = aL*normq + theta*Dm;
}
...
extern "C"
{
  ...
  void calculateResidual_LADR1Dellam (double theta,
				      double aL,
				      double Dm,
				      double dtnp1,
  ...
  void calculateJacobian_LADR1Dellam(double theta,
				     double aL,
				     double Dm,
				     double dtnp1,
  ...
  void markInflowBoundaryPoints(double tn, 
				double tnp1,
				double t,
				int nExteriorElementBoundaries_global,
				int nQuadraturePoints_elementBoundary,
  ...
  void accumulateInflowFluxInGlobalResidual(int nElements_global,//mesh representation
					    int nNodes_global,
					    int nNodes_element,
					    int nElementBoundaries_element,
					    int nExteriorElementBoundaries_global,
