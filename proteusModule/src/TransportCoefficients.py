...
class GroundwaterTransportCoefficientsELLAM(TC_base):
    ...
    TODO
       decide if this is the best place to compute the trackingVelocity (i.e., poreVelocity)
       or if that should go in the Tracking class
    ...
    def __init__(self,nc=1,
        ...
        self.velocitySpace = {0:None}
        self.characteristic_velocitySpace = {0:None}; self.adjoint_velocitySpace = {0:None}
        self.adjoint_velocity_times_last = {0:None};  self.adjoint_velocity_times={0:None}
        self.adjoint_velocity_dofs_last = {0:None};   self.adjoint_velocity_dofs = {0:None}
        self.adjoint_velocity_l2g = {0:None}; 
    ...
    def initializeMesh(self,mesh):
        ...
        if self.velocitySpaceFlag == 'c0p1':
            self.velocitySpace  = {0:FemTools.C0_AffineLinearOnSimplexWithNodalBasis(mesh,self.nd)}
            self.velocity      = {0:FemTools.FiniteElementFunction(self.velocitySpace[0],dim_dof=self.nd,isVector=True,name="velocity")}
            self.velocity_interpolation_values = {0:numpy.zeros((mesh.nElements_global,
                                                                 self.velocitySpace[0].referenceFiniteElement.interpolationConditions.nQuadraturePoints,
                                                                 self.nd),'d')}
            self.velocity_dofs = {0:self.velocity[0].dof}
            self.velocity_l2g  = {0:self.velocitySpace[0].dofMap.l2g}
            #adjoint/characteristic speed might be discontinuous at element boundaries if heterogeneous
            self.adjoint_velocitySpace= {0:FemTools.DG_AffineLinearOnSimplexWithNodalBasis(mesh,self.nd)}
            self.adjoint_velocity     = {0:FemTools.FiniteElementFunction(self.adjoint_velocitySpace[0],dim_dof=self.nd,isVector=True,name="adjoint_velocity")}
            self.adjoint_velocity_interpolation_values = {0:numpy.zeros((mesh.nElements_global,
                                                                         self.adjoint_velocitySpace[0].referenceFiniteElement.interpolationConditions.nQuadraturePoints,
                                                                         self.nd),'d')}
            self.adjoint_velocity_dofs = {0:self.adjoint_velocity[0].dof}
            self.adjoint_velocity_l2g  = {0:self.adjoint_velocitySpace[0].dofMap.l2g}
            #
            ...
            self.velocity_dofs = {0:numpy.zeros((self.mesh.nElements_global*self.nVDOFs_element),'d')}
            self.velocity_l2g  = {0:numpy.arange((self.mesh.nElements_global*self.nVDOFs_element),dtype='i').reshape((mesh.nElements_global,self.nVDOFs_element))}
            self.velocity_interpolation_values = {0:numpy.zeros((mesh.nElements_global,self.nVDOFs_element),'d')}
            self.need_ebq_arrays = True
            #
            self.adjoint_velocity_dofs[0] = numpy.copy(self.velocity_dofs[0])
    ...
    def initializeElementQuadrature(self,t,cq):
        ...
        self.q[('velocity',0)] = numpy.zeros((cq['x'].shape[0],cq['x'].shape[1],self.nd),'d')
        if self.velocitySpace[0] != None:
            self.q[('v',0)]= numpy.zeros((cq['x'].shape[0],cq['x'].shape[1],
                                          self.velocitySpace[0].referenceFiniteElement.localFunctionSpace.dim),
                                         'd')
            #
            self.velocitySpace[0].getBasisValues(self.elementQuadraturePoints,self.q[('v',0)])
        #could reuse trial functions from own transport class in some cases
        self.q[('adjoint_velocity',0)] = numpy.zeros((cq['x'].shape[0],cq['x'].shape[1],self.nd),'d')
        if self.adjoint_velocitySpace[0] != None:
            self.q[('adjoint_velocity_v',0)]= numpy.zeros((cq['x'].shape[0],cq['x'].shape[1],
                                                           self.adjoint_velocitySpace[0].referenceFiniteElement.localFunctionSpace.dim),
                                                          'd')
            #
            self.adjoint_velocitySpace[0].getBasisValues(self.elementQuadraturePoints,self.q[('adjoint_velocity_v',0)])
        self.q[('characteristic_velocity',0)] = self.q[('adjoint_velocity',0)]
    
    def initializeGlobalExteriorElementBoundaryQuadrature(self,t,cebqe):
        """
        Give the TC object access to the exterior element boundary quadrature storage
        """
        self.ebqe[('velocity',0)] = numpy.zeros((cebqe['x'].shape[0],cebqe['x'].shape[1],self.nd),'d')
        if self.velocitySpace[0] != None:
            self.ebqe[('v',0)]= numpy.zeros((cebqe['x'].shape[0],cebqe['x'].shape[1],
                                             self.velocitySpace[0].referenceFiniteElement.localFunctionSpace.dim),
                                            'd')
            self.velocitySpace[0].getBasisValuesGlobalExteriorTrace(self.elementBoundaryQuadraturePoints,self.ebqe[('v',0)])
        #
        self.ebqe[('adjoint_velocity',0)] = numpy.zeros((cebqe['x'].shape[0],cebqe['x'].shape[1],self.nd),'d')
        if self.adjoint_velocitySpace[0] != None:
            self.ebqe[('adjoint_velocity_v',0)]= numpy.zeros((cebqe['x'].shape[0],cebqe['x'].shape[1],
                                                              self.adjoint_velocitySpace[0].referenceFiniteElement.localFunctionSpace.dim),
                                                             'd')
            self.adjoint_velocitySpace[0].getBasisValuesGlobalExteriorTrace(self.elementBoundaryQuadraturePoints,self.ebqe[('adjoint_velocity_v',0)])
    ...
    def initializeElementBoundaryQuadrature(self,t,cebq,cebq_global):
        ...
        """
        self.ebq[('velocity',0)] = numpy.zeros((cebq['x'].shape[0],cebq['x'].shape[1],cebq['x'].shape[2],self.nd),'d')
        self.ebq_global[('velocity',0)] = numpy.zeros((cebq_global['x'].shape[0],cebq_global['x'].shape[1],self.nd),'d')
    ...
    def evaluateVelocity(self,t,q):
        ...
        if self.velocityFunctions != None:
            ...
            if self.velocitySpace[0] != None:
                for eN in range(self.mesh.nElements_global):
                    for k in range(self.velocitySpace[0].referenceFiniteElement.interpolationConditions.nQuadraturePoints):
                        self.velocity_interpolation_values[0][eN,k,:] = self.velocityFunctions[0](self.velocitySpace[0].interpolationPoints[eN,k],t)
                #
                #evaluate velocity degrees of freedom from its interpolation conditions
                self.velocity[0].projectFromInterpolationConditions(self.velocity_interpolation_values[0])
                self.velocity[0].getValues(self.q[('v',0)],self.q[('velocity',0)])
                self.velocity[0].getValuesGlobalExteriorTrace(self.ebqe[('v',0)],self.ebqe[('velocity',0)])
            ...
            #manual evaluation for now
                ...
                if self.need_ebq_arrays:
                    ...
                    if self.velocitySpaceFlag == 'bdm1':
                        ...
                        self.ebq[('velocity',0)] = numpy.zeros((self.mesh.nElements_global,
                                                                self.mesh.nElementBoundaries_element,
                                                                nElementBoundaryQuadraturePoints_elementBoundary,
                ...
                if self.velocitySpaceFlag == 'rt0':
                    ...
                    for eN in range(self.mesh.nElements_global):
                        for ebN in range(self.mesh.nElementBoundaries_element):
                            ...
                            for kb in range(self.ebq['x'].shape[-2]):#nElementBoundaryQuadraturePoints_elementBoundary):
                                v = self.velocityFunctions[0](self.ebq['x'][eN,ebN,kb],t)
                            ...
                            self.velocity_interpolation_values[0][eN,ebN] = integral
                            self.velocity_dofs[0][self.velocity_l2g[0][eN,ebN]] = self.velocity_interpolation_values[0][eN,ebN]
                        #
                    cpostprocessing.getElementRT0velocityValuesFluxRep(self.vt.mesh.nodeArray,
                                                                       self.vt.mesh.elementNodesArray,
                                                                       self.vt.q['abs(det(J))'],
                                                                       self.vt.q['x'],
                                                                       self.velocity_dofs[0],
                                                                       self.q[('velocity',0)])
                   ...
                   #<(\vec v - \vec \hat{v})\cdot \vec n_f,z>_{f} = 0 for faces gamma_f in each element, z in P^1(\gamma_f)
                    ...
                    for eN in range(self.mesh.nElements_global):
                        for ebN in range(self.mesh.nElementBoundaries_element):
                            for kb in range(self.ebq['x'].shape[-2]):#nElementBoundaryQuadraturePoints_elementBoundary):
                                self.ebq[('velocity',0)][eN,ebN,kb,:] = self.velocityFunctions[0](self.ebq['x'][eN,ebN,kb],t)
                    #
                    #mwf debug
                    #import pdb
                    #pdb.set_trace()
                    cpostprocessing.solveLocalBDM1projection(self.BDMprojectionMat_element,
                                                             ...
                                                             self.ebq[('velocity',0)],
                                                             self.velocity_dofs[0])
                    cpostprocessing.getElementBDM1velocityValuesLagrangeRep(self.vt.q[('v',0)],
                                                                            self.velocity_dofs[0],
                                                                            self.q[('velocity',0)])
        ...
        #velocity functions != None
            ...
            assert self.flowModel.conservativeFlux != None and self.flowModel.conservativeFlux.has_key((0))
            ...
            #import pdb
            #pdb.set_trace()
            self.velocity_dofs[0] = self.flowModel.q[('velocity_dofs',0)]
            self.velocity_l2g[0] = self.flowModel.q[('velocity_l2g',0)]
            self.q[('velocity',0)] = self.flowModel.q[('velocity',0)]
            self.ebqe[('velocity',0)] = self.flowModel.ebqe[('velocity',0)]
            evaluatedVelocity = True
    ...
    def evaluateAdjointVelocity(self,t,q):
        ...
        #import pdb
        #pdb.set_trace()
        if self.adjoint_velocity_times[0] != None and tEval > self.adjoint_velocity_times[0]:
            self.adjoint_velocity_times_last[0] = self.adjoint_velocity_times[0]
            self.adjoint_velocity_dofs_last[0].flat[:] = self.adjoint_velocity_dofs[0]
            self.adjoint_velocity_times[0]      = tEval
        if self.velocitySpace[0] != None:
            ...
            self.adjoint_velocity_interpolation_values[0].flat[:] = self.velocity_interpolation_values[0].flat
            ...
            for eN in range(self.mesh.nElements_global):
                ...
                for k in range(q['dV'].shape[1]):
                    ...
                    vol_e   += q['dV'][eN,k]
                self.adjoint_velocity_interpolation_values[0][eN] *= vol_e/(omega_e+1.e-12) 
            #
            #evaluate velocity degrees of freedom from its interpolation conditions
            self.adjoint_velocity[0].projectFromInterpolationConditions(self.adjoint_velocity_interpolation_values[0])
            self.adjoint_velocity[0].getValues(self.q[('adjoint_velocity_v',0)],self.q[('adjoint_velocity',0)])
            self.adjoint_velocity[0].getValuesGlobalExteriorTrace(self.ebqe[('adjoint_velocity_v',0)],self.ebqe[('adjoint_velocity',0)])
        ...
        #manual evaluation for now
            ...
            self.adjoint_velocity_dofs[0].flat[:] = self.velocity_dofs[0].flat
            for eN in range(self.mesh.nElements_global):
                omega_e = 0.0
                vol_e   = 0.0
                for k in range(q['dV'].shape[1]):
                ...
                for i in range(self.nVDOFs_element):
                    self.adjoint_velocity_dofs[0][self.adjoint_velocity_l2g[0][eN,i]] *= vol_e/(omega_e + 1.0e-12)
            if self.velocityFunctions != None:
                tEval = t
                if self.velocitySpaceFlag == 'rt0':
                    cpostprocessing.getElementRT0velocityValuesFluxRep(self.vt.mesh.nodeArray,
                                                                       ...
                                                                       self.vt.q['abs(det(J))'],
                                                                       self.vt.q['x'],
                                                                       self.adjoint_velocity_dofs[0],
                                                                       self.q[('adjoint_velocity',0)])
                    cpostprocessing.getGlobalExteriorElementBoundaryRT0velocityValuesFluxRep(self.vt.mesh.nodeArray,
                                                                                             self.vt.mesh.elementNodesArray,
                                                                                             self.vt.mesh.elementBoundaryElementsArray,
                                                                                             self.vt.mesh.exteriorElementBoundariesArray,
                ...
                tEval = self.flowModel.timeIntegration.t
                if self.flowModel.velocityPostProcessor.vpp_algorithms[0].localVelocityRepresentationFlag == 2:
                    cpostprocessing.getElementRT0velocityValuesFluxRep(self.vt.mesh.nodeArray,
                                                                       self.vt.mesh.elementNodesArray,
                                                                       self.vt.q['abs(det(J))'],
                                                                       self.vt.q['x'],
                    ...
                    cpostprocessing.getGlobalExteriorElementBoundaryRT0velocityValuesFluxRep(self.vt.mesh.nodeArray,
                                                                                             ...
                                                                                             self.adjoint_velocity_dofs[0],
                                                                                             self.ebqe[('adjoint_velocity',0)])
                elif self.flowModel.velocityPostProcessor.vpp_algorithms[0].localVelocityRepresentationFlag == 1:
                ...
                elif self.flowModel.velocityPostProcessor.vpp_algorithms[0].localVelocityRepresentationFlag == 0:
                    #mwf debug
                    #import pdb
                    #pdb.set_trace()
                    cpostprocessing.getElementBDM1velocityValuesLagrangeRep(self.vt.q[('v',0)],
        ...
        #import pdb
        #pdb.set_trace()
        if self.adjoint_velocity_times_last[0] == None:
            self.adjoint_velocity_times_last[0] = tEval
            self.adjoint_velocity_dofs_last[0]  = numpy.copy(self.adjoint_velocity_dofs[0])
        if self.adjoint_velocity_times[0] == None:
            self.adjoint_velocity_times[0] = tEval
    ...
    def attachModels(self,modelList):
        ...
        if self.velocityFunctions == None:
            ...
            self.q[('velocity',0)] = self.flowModel.q[('velocity',0)]
            self.ebqe[('velocity',0)]= self.flowModel.ebqe[('velocity',0)]
            self.ebq[('velocity',0)] = self.flowModel.ebq[('velocity',0)]
            self.ebq_global[('velocity',0)]= self.flowModel.ebq_global[('velocity',0)]
    def evaluate(self,t,c):
        """
        TODO
        ...
        elif self.ebqe[('velocity',0)].shape == c[('df',0,0)].shape:
            v = self.ebqe[('velocity',0)]
        elif self.ebq[('velocity',0)].shape == c[('df',0,0)].shape:
            v = self.ebq[('velocity',0)]
        elif self.ebq_global[('velocity',0)].shape == c[('df',0,0)].shape:
            v = self.ebq_global[('velocity',0)]
        else:
            print c[('df',0,0)].shape
            print "no v---------------------"
            raise RuntimeError
