...
#include <iostream>
#include <cassert>
//extern "C" void calculateResidual_NCLSQ(int nElements_global,
extern "C" void NCLSQ_RES (int nElements_global,
			   double alphaBDF,
			   ...
			   double* ebqe_u)
{
  //using namespace NCLSQ;
  using namespace NCLSQ_NAME;
  //
  ...
  for(int eN=0;eN<nElements_global;eN++)
      ...
      for (int i=0;i<nDOF_test_element;i++)
	{
	  ...
	  // 	  if (k == 0)
	  // 	    {
	  // 	      std::cout<<"NCLSQ res nSpace= "<<nSpace<<"  eN= "<<eN<<" u= "<<u<<" m= "<<m<<" alphaBDF= "<<alphaBDF<<" m_t= "<<m_t<<" dm_t= "<<dm_t<<" dH[0]= "<< dH[0]<<" grad_u[0]="<<grad_u[0]
  ...
  for (int ebNE = 0; ebNE < nExteriorElementBoundaries_global; ebNE++) 
	...
	{ 
	  ...
	  // 	  if (kb >= 0)
	  // 	    {
	  // 	      std::cout<<"NCLSQ res nSpace= "<<nSpace<<" ebNE= "<<ebNE<<" ebN= "<<ebN<<" kb= "<<kb<<" ebNE_kb= "<<ebNE_kb <<" u_ext= "<<u_ext<<" m_ext= "<<m_ext
	  // 		       <<std::endl;
	  // 	    }
	  for (int i=0;i<nDOF_test_element;i++)
	    {
...
//extern "C" void calculateJacobian_NCLSQ(int nElements_global,
extern "C" void NCLSQ_JAC (int nElements_global,
			   double alphaBDF,
			   int lag_shockCapturing,/*mwf not used yet*/
			   double shockCapturingDiffusion,
			   int* u_l2g,
...
//  	  if (k == 0)
// 	    {
// 	      std::cout<<"NCLSQ jac nSpace= "<<nSpace<<" eN= "<<eN<<" u= "<<u<<" m= "<<m<<" alphaBDF= "<<alphaBDF<<" m_t= "<<m_t<<" dm_t= "
