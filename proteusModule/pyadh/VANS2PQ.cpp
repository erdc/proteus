...
extern "C" void calculateResidual_VANS2PQ(int nElements_global,
					 double dt,
					 double eps_rho,
					 double eps_mu,
					 double sigma,
					 ...
					 double waveSource_zm,
					 double waveSource_zp,
					 ...
					 double* q_mom_w_acc,
					 double* q_mass_adv,
					 double* q_mom_u_acc_last, double* q_mom_v_acc_last, double* q_mom_w_acc_last,
					 double* q_velocity_last,
					 //new for lagging smagorinsky turbulence model if compute here
					 // 				  double* q_grad_u,
					 // 				  double* q_grad_v,
					 // 				  double* q_grad_w,
					 // 				  double* q_grad_u_last,
					 // 				  double* q_grad_v_last,
					 // 				  double* q_grad_w_last,
  ...
  for(int eN=0;eN<nElements_global;eN++)
      ...
      for (int i=0;i<nDOF_test_element;i++)
	{
	  ...
	  register double p=0.0,u=0.0,v=0.0,w=0.0,grad_p[nSpace],grad_u[nSpace],grad_v[nSpace],grad_w[nSpace],
	    ...
	    Lstar_w_u[nDOF_test_element],//new
	    Lstar_v_v[nDOF_test_element],
	    Lstar_u_v[nDOF_test_element],//new
	    Lstar_w_v[nDOF_test_element],//new
	    Lstar_w_w[nDOF_test_element],
	    Lstar_u_w[nDOF_test_element],//new
	    Lstar_v_w[nDOF_test_element],//new
...
// 	    }
          ...
          evaluateCoefficients_c(nonlinearDragFactor,
				 ...
				 mom_w_ham,
				 dmom_w_ham_grad_p);          
// 	  //new turbulence closure, need a way to pick other models
// 	  evaluateTurbulenceClosure_c(smagorinskyCoefficient,
// 				      elementDiameter[eN],
// 				      &grad_u_last[eN_k_nSpace],
// 				      &grad_v_last[eN_k_nSpace],
// 				      &grad_w_last[eN_k_nSpace],
// 				      nu_t);
	  ...
	  evaluateWaveForcing_c(t,
          ...
          //
          backwardEuler_c(dt,
			  q_mom_u_acc_last[eN_k],
			  mom_u_acc,
			  dmom_u_acc_u,
			  mom_u_acc_t,
			  dmom_u_acc_u_t);
          backwardEuler_c(dt,
			  q_mom_v_acc_last[eN_k],
			  mom_v_acc,
			  dmom_v_acc_v,
			  mom_v_acc_t,
			  dmom_v_acc_v_t);
          backwardEuler_c(dt,
			  q_mom_w_acc_last[eN_k],
			  mom_w_acc,
			  dmom_w_acc_w,
			  mom_w_acc_t,
			  dmom_w_acc_w_t);
	  ...
	  pdeResidual_w= Mass_strong_c(mom_w_acc_t) + 
		...
		+ Reaction_adjoint_c(dmom_u_source_u,vel_test_dV[eN_k_i]);//new check and lag
	      ...
	      Lstar_w_u[i]=Reaction_adjoint_c(dmom_u_source_w,vel_test_dV[eN_k_i]);//new check and lag
		...
		+ Reaction_adjoint_c(dmom_v_source_v,vel_test_dV[eN_k_i]);//new check and lag
	      ...
	      Lstar_w_v[i]=Reaction_adjoint_c(dmom_v_source_w,vel_test_dV[eN_k_i]);//new check and lag
	      
	      Lstar_w_w[i]=Advection_adjoint_c(&q_velocity_last[eN_k_nSpace],&vel_grad_test_dV[eN_k_i_nSpace])
		+ Reaction_adjoint_c(dmom_w_source_w,vel_test_dV[eN_k_i]);//new check and lag
	      Lstar_u_w[i]=Reaction_adjoint_c(dmom_w_source_u,vel_test_dV[eN_k_i]);
	      Lstar_v_w[i]=Reaction_adjoint_c(dmom_w_source_v,vel_test_dV[eN_k_i]);
		...
		SubgridError_c(subgridError_w,Lstar_w_u[i]) + //new
		...
		SubgridError_c(subgridError_u,Lstar_u_v[i]) +//new
 		SubgridError_c(subgridError_w,Lstar_w_v[i]) +//new
		...
		SubgridError_c(subgridError_u,Lstar_u_w[i]) + //new
		SubgridError_c(subgridError_v,Lstar_v_w[i]) + //new
...
// 	      q_mass_adv[eN_k_I] = mass_adv[I];                     
// 	    }
	  //TODO add array to save momentum derivatives for lagging, won't be necessary if only linear resistance though
	}
      //
  ...
  for (int ebNE = 0; ebNE < nExteriorElementBoundaries_global; ebNE++) 
	...
	{ 
	  ...
	  register double p_ext=0.0,
	    ...
	    dmom_u_source_w_ext=0.0,
	    dmom_v_source_u_ext=0.0,
	    dmom_v_source_v_ext=0.0,
	    dmom_v_source_w_ext=0.0,
	    dmom_w_source_u_ext=0.0,
	    dmom_w_source_v_ext=0.0,
	    dmom_w_source_w_ext=0.0,
	    mom_u_ham_ext=0.0,
	    ...
	    bc_dmom_u_source_w_ext=0.0,
	    bc_dmom_v_source_u_ext=0.0,
	    bc_dmom_v_source_v_ext=0.0,
	    bc_dmom_v_source_w_ext=0.0,
	    bc_dmom_w_source_u_ext=0.0,
	    bc_dmom_w_source_v_ext=0.0,
	    bc_dmom_w_source_w_ext=0.0,
	  ...
	  evaluateCoefficients_c(nonlinearDragFactor,//new 
				 ...
				 bc_dmom_u_source_w_ext,
				 bc_dmom_v_source_u_ext,
				 bc_dmom_v_source_v_ext,
				 bc_dmom_v_source_w_ext,
				 bc_dmom_w_source_u_ext,
				 bc_dmom_w_source_v_ext,
				 bc_dmom_w_source_w_ext,
				 bc_mom_u_ham_ext,
	  ...
	  exteriorNumericalDiffusiveFlux_c(sdInfo_u_u_rowptr,
					   sdInfo_u_u_colind,
					   isDOFBoundary_u[ebNE_kb],
					   ...
					   flux_mom_u_diff_ext);
	  exteriorNumericalDiffusiveFlux_c(sdInfo_v_v_rowptr,
					   sdInfo_v_v_colind,
					   isDOFBoundary_v[ebNE_kb],
					   ...
					   ebqe_penalty_ext[ebNE_kb],
					   flux_mom_v_diff_ext);
	  exteriorNumericalDiffusiveFlux_c(sdInfo_w_w_rowptr,
					   sdInfo_w_w_colind,
					   isDOFBoundary_w[ebNE_kb],
...
extern "C" void calculateJacobian_VANS2PQ(int nElements_global,
				  double dt,
				  double eps_rho,
				  double eps_mu,
				  double sigma,
  ...
  for(int eN=0;eN<nElements_global;eN++)
	...
	for (int j=0;j<nDOF_trial_element;j++)
	  ...
	  register double p=0.0,u=0.0,v=0.0,w=0.0,
	    ...
	    dpdeResidual_u_v[nDOF_trial_element],dpdeResidual_u_w[nDOF_trial_element],//new
	    dpdeResidual_v_u[nDOF_trial_element],dpdeResidual_v_w[nDOF_trial_element],//new
	    dpdeResidual_w_u[nDOF_trial_element],dpdeResidual_w_v[nDOF_trial_element],//new
	    Lstar_u_p[nDOF_test_element],
	    Lstar_v_p[nDOF_test_element],
	    Lstar_w_p[nDOF_test_element],
	    Lstar_u_u[nDOF_test_element],
	  ...
	  for (int I=0;I<nSpace;I++)
		...
		ReactionJacobian_strong_c(dmom_u_source_u,vel_trial[eN_k_j]);//new check and lag
	      ...
	      dpdeResidual_u_w[j]=ReactionJacobian_strong_c(dmom_u_source_w,vel_trial[eN_k_j]); //new check and lag
		...
		ReactionJacobian_strong_c(dmom_v_source_v,vel_trial[eN_k_j]);//new check and lag
	      ...
	      dpdeResidual_v_w[j]=ReactionJacobian_strong_c(dmom_v_source_w,vel_trial[eN_k_j]); //new check and lag
		...
		AdvectionJacobian_strong_c(&q_velocity_last[eN_k_nSpace],&vel_grad_trial[eN_k_j_nSpace]) +
		ReactionJacobian_strong_c(dmom_w_source_w,vel_trial[eN_k_j]);//new check and lag
	      dpdeResidual_w_u[j]=ReactionJacobian_strong_c(dmom_w_source_u,vel_trial[eN_k_j]); //new check and lag
	      dpdeResidual_w_v[j]=ReactionJacobian_strong_c(dmom_w_source_v,vel_trial[eN_k_j]); //new check and lag
          ...
          calculateSubgridErrorDerivatives_tauRes_c(tau_0,
						    ...
						    dpdeResidual_u_w,//new
						    dpdeResidual_v_p,
						    dpdeResidual_v_v,
						    dpdeResidual_v_u,//new
						    dpdeResidual_v_w,//new
						    dpdeResidual_w_p,
						    dpdeResidual_w_w,
						    dpdeResidual_w_u,//new
						    dpdeResidual_w_v,//new
						    dsubgridError_p_u,
						    dsubgridError_p_v,
						    dsubgridError_p_w,
						    dsubgridError_u_p,
						    ...
						    dsubgridError_u_w,//new
						    dsubgridError_v_p,
						    dsubgridError_v_v,
						    dsubgridError_v_u,//new
						    dsubgridError_v_w,//new
						    dsubgridError_w_p,
						    dsubgridError_w_w,
						    dsubgridError_w_u,//new
						    dsubgridError_w_v);//new
		...
		{ 
		  ...
		  elementJacobian_u_u[i][j] += MassJacobian_weak_c(dmom_u_acc_u_t,vel_trial[eN_k_j],vel_test_dV[eN_k_i]) + 
		    ...
		    SubgridErrorJacobian_c(dsubgridError_w_u[j],Lstar_w_u[i]) +  //new
		  ...
		  elementJacobian_u_v[i][j] += AdvectionJacobian_weak_c(dmom_u_adv_v,vel_trial[eN_k_j],&vel_grad_test_dV[eN_k_i_nSpace]) + 
		    ...
		    SubgridErrorJacobian_c(dsubgridError_v_v[j],Lstar_v_u[i]) + //new
		    SubgridErrorJacobian_c(dsubgridError_w_v[j],Lstar_w_u[i]) ; //new
		  ...
		  elementJacobian_u_w[i][j] += AdvectionJacobian_weak_c(dmom_u_adv_w,vel_trial[eN_k_j],&vel_grad_test_dV[eN_k_i_nSpace]) + 
		    ...
		    ReactionJacobian_weak_c(dmom_u_source_w,vel_trial[eN_k_j],vel_test_dV[eN_k_i]) + //new
		    SubgridErrorJacobian_c(dsubgridError_p_w[j],Lstar_p_u[i]) + 
		    SubgridErrorJacobian_c(dsubgridError_u_w[j],Lstar_u_u[i]) + //new
		    SubgridErrorJacobian_c(dsubgridError_v_w[j],Lstar_v_u[i]) + //new
		    SubgridErrorJacobian_c(dsubgridError_w_w[j],Lstar_w_u[i]); //new
		  ...
		  elementJacobian_v_u[i][j] += AdvectionJacobian_weak_c(dmom_v_adv_u,vel_trial[eN_k_j],&vel_grad_test_dV[eN_k_i_nSpace]) + 
		    ...
		    SubgridErrorJacobian_c(dsubgridError_u_u[j],Lstar_u_v[i]) + //new
		    SubgridErrorJacobian_c(dsubgridError_w_u[j],Lstar_w_v[i]); //new
		  ...
		  elementJacobian_v_v[i][j] += MassJacobian_weak_c(dmom_v_acc_v_t,vel_trial[eN_k_j],vel_test_dV[eN_k_i]) + 
		    ...
		    SubgridErrorJacobian_c(dsubgridError_u_v[j],Lstar_u_v[i]) + //new
		    SubgridErrorJacobian_c(dsubgridError_w_v[j],Lstar_w_v[i]) + //new
		  ...
		  elementJacobian_v_w[i][j] += AdvectionJacobian_weak_c(dmom_v_adv_w,vel_trial[eN_k_j],&vel_grad_test_dV[eN_k_i_nSpace]) +  
		    ...
		    ReactionJacobian_weak_c(dmom_v_source_w,vel_trial[eN_k_j],vel_test_dV[eN_k_i]) + //new
		    SubgridErrorJacobian_c(dsubgridError_p_w[j],Lstar_p_v[i]) +
		    SubgridErrorJacobian_c(dsubgridError_v_w[j],Lstar_v_v[i]) + //new
		    SubgridErrorJacobian_c(dsubgridError_u_w[j],Lstar_u_v[i]) + //new
		    SubgridErrorJacobian_c(dsubgridError_w_w[j],Lstar_w_v[i]);  //new
		  ...
		  elementJacobian_w_u[i][j] += AdvectionJacobian_weak_c(dmom_w_adv_u,vel_trial[eN_k_j],&vel_grad_test_dV[eN_k_i_nSpace]) +  
		    ...
		    ReactionJacobian_weak_c(dmom_w_source_u,vel_trial[eN_k_j],vel_test_dV[eN_k_i]) + //new
		    SubgridErrorJacobian_c(dsubgridError_p_u[j],Lstar_p_w[i]) +
		    SubgridErrorJacobian_c(dsubgridError_u_u[j],Lstar_u_w[i]) + //new
		    SubgridErrorJacobian_c(dsubgridError_v_u[j],Lstar_v_w[i]) + //new
		    SubgridErrorJacobian_c(dsubgridError_w_u[j],Lstar_w_w[i]);  //new
		  ...
		  elementJacobian_w_v[i][j] += AdvectionJacobian_weak_c(dmom_w_adv_v,vel_trial[eN_k_j],&vel_grad_test_dV[eN_k_i_nSpace]) + 
		    ...
		    ReactionJacobian_weak_c(dmom_w_source_v,vel_trial[eN_k_j],vel_test_dV[eN_k_i]) + //new
		    SubgridErrorJacobian_c(dsubgridError_p_v[j],Lstar_p_w[i]) +
		    SubgridErrorJacobian_c(dsubgridError_u_v[j],Lstar_u_w[i]) + //new
		    SubgridErrorJacobian_c(dsubgridError_v_v[j],Lstar_v_w[i]) + //new
		    SubgridErrorJacobian_c(dsubgridError_w_v[j],Lstar_w_w[i]);  //new
		  ...
		  elementJacobian_w_w[i][j] += MassJacobian_weak_c(dmom_w_acc_w_t,vel_trial[eN_k_j],vel_test_dV[eN_k_i]) + 
		    ...
		    ReactionJacobian_weak_c(dmom_w_source_w,vel_trial[eN_k_j],vel_test_dV[eN_k_i]) + //new
		    SubgridErrorJacobian_c(dsubgridError_p_w[j],Lstar_p_w[i]) + 
		    SubgridErrorJacobian_c(dsubgridError_w_w[j],Lstar_w_w[i]) + 
		    SubgridErrorJacobian_c(dsubgridError_u_w[j],Lstar_u_w[i]) + //new
		    SubgridErrorJacobian_c(dsubgridError_v_w[j],Lstar_v_w[i]) + //new
  ...
  for (int ebNE = 0; ebNE < nExteriorElementBoundaries_global; ebNE++) 
	...
	{ 
	  ...
	  for (int j=0;j<nDOF_trial_element;j++)
		...
		ExteriorNumericalDiffusiveFluxJacobian_c(sdInfo_u_u_rowptr,
							 sdInfo_u_u_colind,
							 isDOFBoundary_u[ebNE_kb],
							 &ebqe_n_ext[ebNE_kb_nSpace],
							 mom_u_diff_ten_ext,
		...
		ExteriorNumericalDiffusiveFluxJacobian_c(sdInfo_v_v_rowptr,
							 sdInfo_v_v_colind,
							 isDOFBoundary_v[ebNE_kb],
							 &ebqe_n_ext[ebNE_kb_nSpace],
							 mom_v_diff_ten_ext,
		...
		ExteriorNumericalDiffusiveFluxJacobian_c(sdInfo_w_w_rowptr,
							 sdInfo_w_w_colind,
							 isDOFBoundary_w[ebNE_kb],
							 &ebqe_n_ext[ebNE_kb_nSpace],
							 mom_w_diff_ten_ext,
