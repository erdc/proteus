...
/***********************************************************************
  ...
  TODO:
       add element normals for shape function evaluations --> done
       full tensor diffusion coefficient --> done
...
extern "C" void calculateResidual_LADR2Dellam(double theta,    //physical parameters
					      double aL,
					      double aT,
					      double Dm,
  ...
  for(int eN=0;eN<nElements_global;eN++)
      ...
      for (int i=0;i<nDOF_test_element;i++)
	{
	  ...
	  register double u=0.0,grad_u[nSpace],
	    ...
	    f[nSpace],df[nSpace],a[nSpace*nSpace],//todo make full tensor
	    dm_t=0.0;
          //
	  ...
	  calculateCourantNumber_c(elementDiameter[eN],dm,df,q_cfl[eN_k]);
		...
		eN_k_i_nSpace = eN_k_i*nSpace;
	      //mwf debug
	      //std::cout<<"ellam resid eN= "<<eN<<" k= "<<k<<" m= "<<m<<" dt= "<<x_dt[eN_k]<<" u_test= "<<u_test_dV[eN_k_i]<<" a= ["<<a[0]<<","<<a[1]<<","<<a[2]<<","<<a[3]
	      //	       <<"] grad_u= ["<<grad_u[0]<<","<<grad_u[1]<<"] " <<std::endl;
	  ...
	  //mwf debug
	  //std::cout<<"LADR2Dellam new mass + diffusion globalResid["<<offset_u+stride_u*u_l2g[eN_i]<<"]= "<< globalResidual[offset_u+stride_u*u_l2g[eN_i]] 
	  //	   <<" localRes = "<<elementResidual_u[i] <<std::endl;
        }//i
    }//elements
...
// 		       <<std::endl;
	      evaluateTestFunctionsOnElement(&x_track[eN_k*3],
					     ...
					     &nodeArray[elementNodesArray[eN_track*nNodes_element+2]*3],
					     &elementBoundaryOuterNormalsArray[eN_track*nElementBoundaries_element*nSpace+0*nSpace],
					     &elementBoundaryOuterNormalsArray[eN_track*nElementBoundaries_element*nSpace+1*nSpace],
					     &elementBoundaryOuterNormalsArray[eN_track*nElementBoundaries_element*nSpace+2*nSpace],
					     w);
	      for(int i=0;i<nDOF_test_element;i++) 
		{ 
		  ...
		  //mwf debug
		  //std::cout<<"LADR2Dellam oldMass globalResid["<<offset_u+stride_u*u_l2g[eN_track_i]<<"]= "<< globalResidual[offset_u+stride_u*u_l2g[eN_track_i]] 
...
extern "C" void calculateJacobian_LADR2Dellam (double theta,    //physical parameters
					       double aL,
					       double aT,
					       double Dm,
					       double dtnp1,
	  ...
	  for (int I=0;I<nSpace;I++)
 	  ...
 	  for(int i=0;i<nDOF_test_element;i++)
		...
		{ 
		  ...
		  //mwf debug
		  //std::cout<<"ellam2d jac eN= "<<eN<<" k= "<<k<<" dm= "<<dm<<" dt= "<<x_dt[eN_k]<<" a= ["<<a[0]
		  //   <<","<<a[1]<<","<<a[2]<<","<<a[3]<<"]"<<std::endl;
...
/**********************************************************************
  ...
  velocity is inflow
 **********************************************************************/
extern "C" void markInflowBoundaryPoints2d(double tn, 
					   double tnp1,
					   double t,
					   int nExteriorElementBoundaries_global,
					   int nQuadraturePoints_elementBoundary,
 ...
 **********************************************************************/
extern "C" void accumulateInflowFluxInGlobalResidual2d(int nElements_global,//mesh representation
						     int nNodes_global,
						     int nNodes_element,
						     int nElementBoundaries_element,
						     int nExteriorElementBoundaries_global,
