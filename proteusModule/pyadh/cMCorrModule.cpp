...
static PyObject* cMCorr_calculateResidual(PyObject* self,
					   PyObject* args)
{
  ...
  //calculateResidual_MCorr(nElements_global,
...
static PyObject* cMCorr_calculateJacobian(PyObject* self,
					 PyObject* args)
{
  int nElements_global;
  double epsHeaviside,epsDirac,epsDiffusion;
  ...
  if (!PyArg_ParseTuple(args,
			...
			&globalJacobian))
    return NULL;
  //calculateJacobian_MCorr(nElements_global,
...
static PyMethodDef cMCorrMethods[] = {
 { "calculateResidual",
    cMCorr_calculateResidual,
   METH_VARARGS, 
   "Calculate the global residual for the non-conservative level set equation"},
 { "calculateJacobian",
    cMCorr_calculateJacobian,
   METH_VARARGS, 
...
{
  PyObject *m,*d;
  m = Py_InitModule("cMCorr", cMCorrMethods);
