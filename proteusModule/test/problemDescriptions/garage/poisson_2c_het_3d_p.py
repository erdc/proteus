...
class velEx:
    ...
    def uOfX(self,X):
        ...
        A  = Numeric.reshape(self.aex(X),(nd,nd))
        #mwf debug
...
def getAdvFluxBC1(x):
    if x[0] == 0.0:
        n = Numeric.array([-1.0,0.0,0.0],Numeric.Float)
        #mwf debug
        #print """getAdvFlux1 x= %s q.n= %s """ % (x,Numeric.dot(velEx(u1Ex(),a1).uOfXT(x,0.0),n))
...
def getAdvFluxBC2(x):
    if x[2] == 1.0:
        n = Numeric.zeros((nd,),Numeric.Float); n[2]=1.0
        #mwf debug
        #print """getAdvFlux2 x= %s q.n= %s """ % (x,Numeric.dot(velEx(u2Ex(),a2).uOfXT(x,0.0),n))        
...
def a4(x):
    return Numeric.array([[sin(2.0*pi*x[0])**2 + 5.0,0.0,0.0],[0.0,cos(2.0*pi*x[1])**2 + 5.0,0.0],
...
def a5(x):
    return Numeric.array([[x[0] + 5.0,0.0,0.0],[0.0,x[1] + 5.0,0.0],
...
def a6(x):
    return Numeric.array([[x[0]**2 + 5.0,0.0,0.0],[0.0,x[1]**2 + 5.0,0.0],
...
def a8(x):
    return Numeric.array([[x[1]*x[0]**2 + 5.0,0.0,0.0],[0.0,x[2]*x[1]**2 + 5.0,0.0],
                          [0.0,0.0,x[0]*x[2]**2+5.0]],Numeric.Float)
def f8(x):
