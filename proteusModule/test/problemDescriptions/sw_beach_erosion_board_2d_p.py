...
def bathymetryGrad(x):
    if x[0] <= x_bs:  return (0.0,0.)
    if x[0] <= x_be:  return (0.1,0.) #beach slope
    if x[0] <= x_se:  return (1.0/s,0.)
    if x[0] <= x_ce:  return (0.0,0.)
    if x[0] <= x_bse: return (1./s_2,0.)
    return (0.0,0.)
...
def getDBC_h(x,flag):
    if x[0] <= domain.x[0] + 1.0e-8:
        return lambda x,t:h_swd +H*math.sin(t/T_s)
...
def getAFBC_h(x,flag):
    if x[1] <= domain.x[1] + 1.0e-8:
        return lambda x,t: 0.0
    if x[1] >= domain.L[1] - 1.0e-8:
        return lambda x,t: 0.0
