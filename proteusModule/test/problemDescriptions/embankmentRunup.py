...
from embankment2dDomain import *
#numerics options
useVANS2P2D=True
useNCLS  =True
useVOF   =True
...
#spatial domain
nd = 2
polyfile = 'cobras_saj_embankment'
...
#outflowLength= 1.0
#domainHeight = 15.0
#lengthBousDomain = 100.0
inflowLength = 3.0#30.0#5.0
inflowPad    = 5.0#30.5#5.5#8.0
outflowLength= 1.0
domainHeight = 10.0
lengthBousDomain = 170.0#130.0,170.0
ransDomainStop = 230.0

domain = embankment2d(lengthBousDomain = lengthBousDomain,#default 170
                      ransDomainStop = ransDomainStop,#default >= 250
                      ransDomainHeight = domainHeight,
                      inflowLength = inflowLength,
                      inflowPad    = inflowPad,
                      outflowLength= outflowLength)
...
waterLevelBase = domain.crestHeight-waveAmplitude*0.5
waveHeight=1.0
wavePeriod= 10.0   #[s]
waveCelerity  = 1.5#[m/s] don't know
waveNumber= 1.04/waterLevelBase #[1/m] not used
...
source_width =max(3.0*dx,0.5)#max(3.0*dx,0.1)
...
import math
T = 0.3#wavePeriod*4.0#*10.0#10.#*10.0#2.5#3.0#10.0
...
spongeLayerWidth = max(2.0*dx,0.5)#0.2*domain.inflowLength)
