import proteus
from proteus.nondilutetransport.cEVNonDiluteTransport import *
...
class ShockCapturing(proteus.ShockCapturing.ShockCapturing_base):
    def __init__(self,coefficients,nd,shockCapturingFactor=0.25,lag=True,nStepsToDelay=None):
        ...
        self.nSteps=0
        if self.lag:
            logEvent("EVNonDiluteTransport.ShockCapturing: lagging requested but must lag the first step; switching lagging off and delaying")
    ...
    def updateShockCapturingHistory(self):
        ...
        if self.lag == False and self.nStepsToDelay is not None and self.nSteps > self.nStepsToDelay:
            logEvent("EVNonDiluteTransport.ShockCapturing: switched to lagged shock capturing")
            self.lag = True
            self.numDiff_last=[]
            for ci in range(self.nc):
                self.numDiff_last.append(self.numDiff[ci].copy())
        logEvent("EVNonDiluteTransport: max numDiff %e" % (globalMax(self.numDiff_last[0].max()),))
...
class Coefficients(proteus.TransportCoefficients.TC_base):
    ...
    def preStep(self,t,firstStep=False):
        ...
        if self.checkMass:
            ...
            logEvent("Phase  0 mass before EVNonDiluteTransport step = %12.5e" % (self.m_pre,),level=2)
        copyInstructions = {}
    ...
    def postStep(self,t,firstStep=False):
        ...
        if self.checkMass:
            self.m_post = Norms.scalarDomainIntegral(self.model.q['dV'],
                                                     self.model.q[('m',0)],
                                                     self.model.mesh.nElements_owned)
            logEvent("Phase  0 mass after EVNonDiluteTransport step = %12.5e" % (self.m_post,),level=2)
        copyInstructions = {}
        return copyInstructions
...
class LevelModel(proteus.Transport.OneLevelTransport):
    ...
    def __init__(self,
        ...
        compKernelFlag=0
        self.evnondilutetransport = cEVNonDiluteTransport_base(self.nSpace_global,
