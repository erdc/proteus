...
import sys # mql
from heapq import heappush, heappop # mql
...
class Gauges(AV_base):
    ...
    #####################################################################################
    def minkowski_distance_p(self,x, y, p=2):
        """
    ...
    def myQuery(self, x, k=1, eps=0, p=2, distance_upper_bound=np.inf):
        # pseudo constructor #
        data = self.model.levelModelList[-1].mesh.nodeArray
        leafsize=10
...
###############################
    ...
    def getLocalNearestNode(self, location):
        ...
        #nearest_node_distance_kdtree, nearest_node_kdtree = self.nodes_kdtree.query(location) #mql
        nearest_node_distance_kdtree, nearest_node_kdtree = self.myQuery(location)
    ...
    def attachModel(self, model, ar):
        ...
        #self.nodes_kdtree = spatial.cKDTree(model.levelModelList[-1].mesh.nodeArray)# mql
        linesSegments = []
