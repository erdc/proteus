...
class Transport_Dilute_Dispersion(LinearVADR_ConstantCoefficients):
	...
	def __init__(self, nc=1, poro = 0, mass=0, diff=0, alpha_L=0, diff_ModelId = None, adv_ModelId = None, mom_ModelId = None):
		mass={}
		advection={}
		diffusion={}
		potential={}
	...
	def attachModels(self,modelList):
		...
		self.q_vel  = self.mom_Model.q[('velocity',0)]
		self.ebqe_vel = self.mom_Model.ebqe[('velocity',0)]
		if self.mom_Model.ebq.has_key(('velocity',0)):
			self.ebq_vel = self.mom_Model.ebq[('velocity',0)]
		if self.mom_Model.ebq_global.has_key(('velocity',0)):
			self.ebq_global_vel = self.mom_Model.ebq_global[('velocity',0)] 
	...
	def preStep(self,t,firstStep=True):
		...
		print "Diffusion Pre",t,self.diff_Model.u[0].dof[0]
		...
		#self.diff_Model.timeIntegration.m_ip_last = self.adv_Model.timeIntegration.m_ip_tmp
		#self.diff_Model.calculateCoefficients()
		#self.diff_Model.calculateElementResidual()

		#self.diff_Model.timeIntegration.m_last[0].flat[:] = self.diff_Model.timeIntegration.m_tmp[0].flat
		...
		print "Diffusion Pre_Final",t,self.diff_Model.u[0].dof[0]
		#return copyInstructions

	def postStep(self,t,firstStep=False):
		print "Diffusion Post",t,self.diff_Model.u[0].dof[0]

	def evaluate(self,t,c):
		#import pdb; pdb.set_trace()
		#print "Diffusion",t,c[('u',0)][0]
...
class Porous_Media_Flow(TC_base):
	...
	def __init__(self,nc=1,nd=1,diff_ModelId = None, adv_ModelId = None, mom_ModelId = None, K = 0.0 , grav = -980.):
		mass={}
		advection={}
		diffusion={}
		potential={}
