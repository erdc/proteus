...
class RKEV(proteus.TimeIntegration.SSP):
    ...
    def choose_dt(self):
        ...
        max_tim = self.cfl.max()
        if max_tim > 1.e8:
            max_tim = -100.
        maxCFL = max(maxCFL,globalMax(max_tim))
...
class LevelModel(proteus.Transport.OneLevelTransport):
    ...
    def FCTStep(self,rowptr,colind, MassMatrix):
        ...
        self.evdilutetransport.FCTStep(
                         ...
                         old_solution,#soln  ##ERROR HERE MAYBE
...
class MyCoefficients(Coefficients):
	...
	def preStep(self,t,firstStep=False):
		#import pdb; pdb.set_trace()
		if self.Alt_Split:
		    self.adv_Model.u_dof_old[:] = self.adv_Model2.u[0].dof[:]
		    self.adv_Model2.u_dof_old[:] = self.diff_Model.u[0].dof[:]
		else:
...
class MyCoefficients_Dimension(MyCoefficients):
	def __init__(self,
                 LS_model=None,
                 V_model=0,
                 RD_model=None,
	...
	def attachModels(self,modelList):
		...
		if self.Alt_Split:
		          self.adv_Model2 = modelList[self.adv_ModelId2]
		self.q_v = np.ones(self.adv_Model.q[('grad(u)',0)].shape,'d')
		self.ebqe_v = np.ones(self.adv_Model.ebqe[('grad(u)',0)].shape,'d')
		self.ebqe_phi = np.ones(self.adv_Model.ebqe[('u',0)].shape,'d')
...
class LevelModel_LinearAdvection(proteus.Transport.OneLevelTransport):
    nCalls=0
    def __init__(self,
                 uDict,
                 phiDict,
    ...
    def getResidual(self,u,r):
        ...
        self.calculateResidual(#element
            ...
            self.coefficients.diff,
            self.coefficients.alpha_L,
            np.zeros_like(self.coefficients.porosity_dof), #TIM FIXXXX
            #self.coefficients.mom_Model.u[0].dof,
            self.entropy,
            self.Alt_Split)
