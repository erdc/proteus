...
"""
Heterogeneous Poisson's equation, -div(a(x)u) = f(x), on unit domain [0,10]x[0,10]x[0,10]
"""
...
def getDBC5(x,flag):
    if x[0] in [0.0] or x[1] in [0.0,10.0] or x[2] in [0.0,10.0]:
...
def getDiffFluxBC5(x,flag):
    ...
    if not (x[0] in [0.0] or x[1] in [0.0,10.0] or x[2] in [0.0,10.0]):
