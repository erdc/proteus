...
domain.PUMIMesh.transferFieldToPUMI("coordinates",mesh.subdomainMesh.nodeArray)
...
vector=numpy.zeros((mesh.subdomainMesh.nNodes_global,3),'d')
dummy = numpy.zeros(mesh.subdomainMesh.nNodes_global); 
vector[:,0] = dummy
vector[:,1] = Uinf*mesh.subdomainMesh.nodeArray[:,2]/Lz #v-velocity
vector[:,2] = dummy
domain.PUMIMesh.transferFieldToPUMI("velocity", vector)
del vector
del dummy

scalar=numpy.zeros((mesh.subdomainMesh.nNodes_global,1),'d')
domain.PUMIMesh.transferFieldToPUMI("p", scalar)

scalar[:,0] = mesh.subdomainMesh.nodeArray[:,2]
domain.PUMIMesh.transferFieldToPUMI("phi", scalar)
del scalar

scalar = numpy.zeros((mesh.subdomainMesh.nNodes_global,1),'d')+1.0
