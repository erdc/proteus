...
namespace proteus
{
  ...
  public:
    ...
    void evaluateCoefficients(const double eps_rho,
			      ...
			      const double& pStar,
			      const double grad_pStar[nSpace],
					...
					double* velocity)
    {
      ...
      flux_vmom = 0.0;
      /* flux_wmom = 0.0; */
      flowDirection=vos*(n[0]*u+n[1]*v);
      speed = fabs(flowDirection);
      bc_flowDirection=vos*(n[0]*bc_u+n[1]*bc_v);
      ...
      for(int eN=0;eN<nElements_global;eN++)
	{
	  ...
	  for(int k=0;k<nQuadraturePoints_element;k++)
		...
		eN_nDOF_trial_element = eN*nDOF_trial_element;
	      register double pStar=0.0,u=0.0,v=0.0,w=0.0,
		grad_pStar[nSpace],grad_u[nSpace],grad_v[nSpace],grad_w[nSpace],
		mom_u_acc=0.0,
		dmom_u_acc_u=0.0,
		mom_v_acc=0.0,
		dmom_v_acc_v=0.0,
		...
		G[nSpace*nSpace],G_dd_G,tr_G,norm_Rv,h_phi, dmom_adv_star[nSpace],dmom_adv_sge[nSpace];
	      ...
	      //get the solution
	      ck.valFromDOF(p_dof,&p_l2g[eN_nDOF_trial_element],&p_trial_ref[k*nDOF_trial_element],pStar);
	      ...
	      ck.gradFromDOF(p_dof,&p_l2g[eN_nDOF_trial_element],p_grad_trial,grad_pStar);
	      ...
	      for (int j=0;j<nDOF_trial_element;j++)
		{
	      ...
	      q_velocity[eN_k_nSpace+0]=vos*u;
	      q_velocity[eN_k_nSpace+1]=vos*v;
	      ...
	      updateDarcyForchheimerTerms_Ergun(/* linearDragFactor, */
						...
						q_velocity_fluid[eN_k_nSpace+1],//cek hackk, should not be used
						mom_u_source,
						mom_v_source,
						mom_w_source,
						dmom_u_source,
		...
		ck.Advection_strong(dmom_adv_sge,grad_u) + //includes non-consrv 
		ck.Hamiltonian_strong(dmom_u_ham_grad_p,grad_pStar) +
		...
		ck.Advection_strong(dmom_adv_sge,grad_v) + //include  non-consrv
		ck.Hamiltonian_strong(dmom_v_ham_grad_p,grad_pStar) + 
		ck.Reaction_strong(mom_v_source) -
	      ...
	      /* 	ck.Hamiltonian_strong(dmom_w_ham_grad_p,grad_pStar) + */
	...
	{ 
	  ...
	  for  (int kb=0;kb<nQuadraturePoints_elementBoundary;kb++) 
		...
		ebN_local_kb_nSpace = ebN_local_kb*nSpace;
	      register double pStar_ext=0.0,
		u_ext=0.0,
		v_ext=0.0,
		w_ext=0.0,
		grad_pStar_ext[nSpace],
		grad_u_ext[nSpace],
		grad_v_ext[nSpace],
		grad_w_ext[nSpace],
		mom_u_acc_ext=0.0,
		...
		force_x,force_y,force_z,force_p_x,force_p_y,force_p_z,force_v_x,force_v_y,force_v_z,r_x,r_y,r_z;
	      ...
	      ck.valFromDOF(p_dof,&p_l2g[eN_nDOF_trial_element],&p_trial_trace_ref[ebN_local_kb*nDOF_test_element],pStar_ext);
	      ...
	      ck.gradFromDOF(p_dof,&p_l2g[eN_nDOF_trial_element],p_grad_trial_trace,grad_pStar_ext);
	      ...
	      for (int j=0;j<nDOF_trial_element;j++)
		{
	      ...
	      bc_pStar_ext = isDOFBoundary_p[ebNE_kb]*ebqe_bc_p_ext[ebNE_kb]+(1-isDOFBoundary_p[ebNE_kb])*pStar_ext;
	      ...
	      if (ebN < nElementBoundaries_owned)
		{
		  ...
		  force_p_x = pStar_ext*normal[0];
		  force_p_y = pStar_ext*normal[1];
		  /* force_p_z = pStar_ext*normal[2]; */
		  ...
		  for(int j=0;j<nDOF_trial_element;j++) 
			...
			ck.NumericalDiffusionJacobian(q_numDiff_v_last[eN_k],&vel_grad_trial[j_nSpace],&vel_grad_test_dV[i_nSpace]); 
		      ...
		      /* elementJacobian_w_w[i][j] += ck.MassJacobian_weak(dmom_w_acc_w_t,vel_trial_ref[k*nDOF_trial_element+j],vel_test_dV[i]) +  */
                      /*   ck.HamiltonianJacobian_weak(dmom_w_ham_grad_w,&p_grad_trial[j_nSpace],vel_test_dV[i]) +  */
	...
	permutations[i]=i;//just to initialize
      ...
      for (int ebNI = 0; ebNI < nInteriorElementBoundaries_global; ebNI++)
		...
		right_ebN_element_kb_nDOF_test_element=(right_ebN_element*nQuadraturePoints_elementBoundary+right_kb)*nDOF_test_element;
    	      ...
    	      //
    	      velocityAverage[ebN_kb_nSpace+0]=0.5*(vos_left*u_left + vos_right*u_right);
    	      velocityAverage[ebN_kb_nSpace+1]=0.5*(vos_left*v_left + vos_right*v_right);
