...
namespace proteus
{
  ...
  public:
    ...
    {
      double mu,nu,H_mu,uc,duc_du,duc_dv,duc_dw,viscosity,H_s,rho,H_rho,rhoScale;
      H_mu = (1.0-useVF)*smoothedHeaviside(eps_mu,phi)+useVF*fmin(1.0,fmax(0.0,vf));
      nu  = nu_0*(1.0-H_mu)+nu_1*H_mu;
      mu  = rho_0*nu_0*(1.0-H_mu)+rho_1*nu_1*H_mu;
      ...
      mom_w_source += H_s*viscosity*(alpha + beta*uc)*(w-w_s)*rhoScale;

      dmom_u_source[0] = H_s*viscosity*(alpha + beta*(uc + u*duc_du))*rhoScale;
      dmom_u_source[1] = H_s*viscosity*beta*u*duc_dv*rhoScale;
      dmom_u_source[2] = H_s*viscosity*beta*u*duc_dw*rhoScale;
    
      dmom_v_source[0] = H_s*viscosity*beta*v*duc_du*rhoScale;
      dmom_v_source[1] = H_s*viscosity*(alpha + beta*(uc + v*duc_dv))*rhoScale;
      dmom_v_source[2] = H_s*viscosity*beta*w*duc_dw*rhoScale;

      dmom_w_source[0] = H_s*viscosity*beta*w*duc_du*rhoScale;
      dmom_w_source[1] = H_s*viscosity*beta*w*duc_dv*rhoScale;
      dmom_w_source[2] = H_s*viscosity*(alpha + beta*(uc + w*duc_dw))*rhoScale;
    }
