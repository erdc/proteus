...
class Coefficients(TransportCoefficients.PoissonEquationCoefficients):
    ...
    def __init__(self, func, nd=2):
        ...
        fOfX = [self.uOfXT]  # scaled function reciprocal
    ...
    def preStep(self, t, firstStep=False):
        ...
        #     area_e, center = get_center_area(self.mesh.nodeArray[self.mesh.elementNodesArray[e]])
        #     integral_1_over_f += area_e/self.myfunc(center)
        #     area += area_e
        # calculating areas
        xx = self.model.q['x']
        ...
        for e in xrange(self.mesh.elementNodesArray.shape[0]):
            ...
            self.areas[e] = area
            self.integral_area += area
        self.C = integral_1_over_f  # update scaling coefficient
        logEvent("Element Quadrature - updating uOfXT",
                 level=3)
        ci = 0
        cq = self.model.q
        for i in range(len(cq[('r',ci)].flat)):
            cq[('r',ci)].flat[i] = -self.uOfXT(cq['x'].flat[3*i:3*(i+1)])
    ...
    def uOfXT(self, x):
        ...
        reaction term
        """
        return 1./(self.myfunc(x)*self.C)-1
        ...
        for j, t in enumerate(t_range):
            ...
            for i in range(len(xx)):
                ...
                if j > 0:
                    ...
                    v_grad = self.grads[i]
                dphi = v_grad/(t*(1./(self.myfunc(phi))*self.C)+(1-t)*1)
                # # RK4
                # # -----
                # phi1 = copy.copy(phi)
                # phi2 = copy.copy(phi1)
