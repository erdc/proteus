...
class ShockCapturing(proteus.ShockCapturing.ShockCapturing_base):
    def __init__(self, coefficients, nd, shockCapturingFactor=0.25, lag=True, nStepsToDelay=1):
        ...
        self.nSteps = 0
        if self.lag:
            logEvent("RDLS.ShockCapturing: lagging requested but must lag the first step; switching lagging off and delaying")
            #Ensure nStepsToDelay value makes sense by throwing an error
            if(self.nStepsToDelay is None or self.nStepsToDelay < 1):
                sys.exit("RDLS.ShockCapturing: nStepsToDelay cannot be None or < 1 with lagging, please specify an integer: nStepsToDelay>=1")
    ...
    def updateShockCapturingHistory(self):
        ...
        if self.lag:
            if self.nSteps > self.nStepsToDelay:
                ...
                logEvent("RDLS.ShockCapturing: switched to lagged shock capturing")
