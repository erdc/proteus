...
class LevelModel(proteus.Transport.OneLevelTransport):
    ...
    def update_variables_qt_after_adaptive(self):#similar to postStep;
        self.u[0].getValues(self.q[('w',0)],self.q[('u',0)])
        self.u[0].getGradientValues(self.q[('grad(w)',0)], self.q[('grad(u)',0)])
        self.u[0].getValuesGlobalExteriorTrace(self.ebqe[('w',0)], self.ebqe[('u',0)])
        self.u[0].getGradientValuesGlobalExteriorTrace(self.ebqe[('grad(w)',0)], self.ebqe[('grad(u)',0)])
        
        r = 1 ###since Backward Euler
        self.q_p_sharp[:] = self.q[('u', 0)] + r * self.coefficients.pressureIncrementModel.q[('u', 0)]
        self.ebqe_p_sharp[:] = self.ebqe[('u', 0)] + r * self.coefficients.pressureIncrementModel.ebqe[('u', 0)]
        self.q_grad_p_sharp[:] = self.q[('grad(u)', 0)] + r * self.coefficients.pressureIncrementModel.q[('grad(u)', 0)]
        self.ebqe_grad_p_sharp[:] = self.ebqe[('grad(u)', 0)] + r * self.coefficients.pressureIncrementModel.ebqe[('grad(u)', 0)]

        self.coefficients.fluidModel.q['p'][:] = self.q_p_sharp
        self.coefficients.preStep(self.timeIntegration.t)
