                                           ...
                                           double *particle_surfaceArea)
    {
                             ...
                             double* isActiveDOF)
      {
		...
		register int eN_i=eN*nDOF_mesh_trial_element+i;
            ...
            int icase = gf.calculate(element_phi, element_nodes, x_ref, 1.,1.,false,false);
#endif
            if (icase == 0)
              {
                //only works for simplices
            ...
            for(int k=0;k<nQuadraturePoints_element;k++)
              {
                ...
                ck.gradFromDOF(p_old_dof,&p_l2g[eN_nDOF_trial_element],p_grad_trial_ib,grad_p_old);
                ck_v.gradFromDOF(u_old_dof,&vel_l2g[eN_nDOF_v_trial_element],vel_grad_trial_ib,grad_u_old);
                ck_v.gradFromDOF(v_old_dof,&vel_l2g[eN_nDOF_v_trial_element],vel_grad_trial_ib,grad_v_old);
                ...
                const double D_s = gf_s.D(particle_eps,phi_solid[eN_k]);
                if ( H_s != 0.0 || D_s != 0.0)
                  {
                    element_active=true;
                ...
                if (NONCONSERVATIVE_FORM > 0.0)
                  {
                    dmom_adv_sge[0] = 0.0;
                    dmom_adv_sge[1] = 0.0;
                    dmom_ham_grad_sge[0] = inertial_term*dmom_u_acc_u*(q_velocity_sge[eN_k_nSpace+0] - MOVING_DOMAIN*xt);
                    dmom_ham_grad_sge[1] = inertial_term*dmom_u_acc_u*(q_velocity_sge[eN_k_nSpace+1] - MOVING_DOMAIN*yt);
                  }
                else
                  {
                    dmom_adv_sge[0] = inertial_term*dmom_u_acc_u*(q_velocity_sge[eN_k_nSpace+0] - MOVING_DOMAIN*xt);
                    dmom_adv_sge[1] = inertial_term*dmom_u_acc_u*(q_velocity_sge[eN_k_nSpace+1] - MOVING_DOMAIN*yt);
                ...
                dmom_adv_star[0] = inertial_term*dmom_u_acc_u*(q_velocity_sge[eN_k_nSpace+0] - MOVING_DOMAIN*xt + useRBLES*subgridError_u);
                dmom_adv_star[1] = inertial_term*dmom_u_acc_u*(q_velocity_sge[eN_k_nSpace+1] - MOVING_DOMAIN*yt + useRBLES*subgridError_v);

                mom_u_adv[0] += inertial_term*dmom_u_acc_u*(useRBLES*subgridError_u*q_velocity_sge[eN_k_nSpace+0]);
                mom_u_adv[1] += inertial_term*dmom_u_acc_u*(useRBLES*subgridError_v*q_velocity_sge[eN_k_nSpace+0]);

                mom_v_adv[0] += inertial_term*dmom_u_acc_u*(useRBLES*subgridError_u*q_velocity_sge[eN_k_nSpace+1]);
                mom_v_adv[1] += inertial_term*dmom_u_acc_u*(useRBLES*subgridError_v*q_velocity_sge[eN_k_nSpace+1]);
                ...
                for(int i=0;i<nDOF_test_element;i++)
                  {
                    ...
                    if (nDOF_test_element == nDOF_v_test_element)
                      {
                        elementResidual_p[i] +=
                          H_s*H_f*(PRESSURE_PROJECTION_STABILIZATION * ck.pressureProjection_weak(mom_uu_diff_ten[1], p, p_element_avg, p_test_ref[k*nDOF_test_element+i], dV) +
            ...
            for(int k=0;k<nQuadraturePoints_element;k++)
              {
                //compute indices and declare local storage
                register int eN_k = eN*nQuadraturePoints_element+k;
                q_numDiff_u[eN_k] = numDiffMax;
                q_numDiff_v[eN_k] = numDiffMax;
                q_numDiff_w[eN_k] = numDiffMax;
              }
            //
            ...
            for  (int kb=0;kb<nQuadraturePoints_elementBoundary;kb++)
              {
                ...
                if (use_ball_as_particle == 1)
                  {
                    get_distance_to_ball(nParticles, ball_center, ball_radius,x_ext,y_ext,z_ext,ebqe_phi_s[ebNE_kb]);
                  }
                        ...
                        if (nDOF_test_element == nDOF_v_trial_element)
                          {
                            elementJacobian_p_p[i][j] += H_s*H_f*((1-PRESSURE_PROJECTION_STABILIZATION)*ck.SubgridErrorJacobian(dsubgridError_u_p[j],Lstar_u_p[i]) +
                                                                  ...
                                                                  PRESSURE_PROJECTION_STABILIZATION*ck.pressureProjection_weak(mom_uu_diff_ten[1], p_trial[j], 1./3., p_test_ref[k*nDOF_test_element +i],dV));
                          }
                      }
                  }
                for(int i=0;i<nDOF_test_element;i++)
                    ...
                    for(int j=0;j<nDOF_v_trial_element;j++)
                      {
                        ...
                        elementJacobian_u_u[i][j] += H_s*H_f*(ck.MassJacobian_weak(dmom_u_acc_u_t,vel_trial[j],vel_test_dV[i]) +
                                                              ...
                                                              ck.NumericalDiffusionJacobian(q_numDiff_u_last[eN_k],&vel_grad_trial_ib[j_nSpace],&vel_grad_test_dV[i_nSpace]));
                    ...
                    for (int j=0;j<nDOF_trial_element;j++)
                      {
                        ...
                        globalJacobian[csrRowIndeces_p_p[eN_i] + csrColumnOffsets_eb_p_p[ebN_i_j]] += H_s*fluxJacobian_p_p[j]*p_test_dS[i];
                      }
                  }
                for (int i=0;i<nDOF_test_element;i++)
                  {
                    ...
                    for (int j=0;j<nDOF_v_trial_element;j++)
                      {
                        ...
                        globalJacobian[csrRowIndeces_p_u[eN_i] + csrColumnOffsets_eb_p_u[ebN_i_j]] += H_s*fluxJacobian_p_u[j]*p_test_dS[i];
                        globalJacobian[csrRowIndeces_p_v[eN_i] + csrColumnOffsets_eb_p_v[ebN_i_j]] += H_s*fluxJacobian_p_v[j]*p_test_dS[i];
                      }
                  }
                for (int i=0;i<nDOF_v_test_element;i++)
                  {
                    ...
                    for (int j=0;j<nDOF_trial_element;j++)
                      {
                        ...
                        globalJacobian[csrRowIndeces_u_p[eN_i] + csrColumnOffsets_eb_u_p[ebN_i_j]] += H_s*fluxJacobian_u_p[j]*vel_test_dS[i];
                        globalJacobian[csrRowIndeces_v_p[eN_i] + csrColumnOffsets_eb_v_p[ebN_i_j]] += H_s*fluxJacobian_v_p[j]*vel_test_dS[i];
                      }
                  }
                for (int i=0;i<nDOF_v_test_element;i++)
                  {
                    ...
                    for (int j=0;j<nDOF_v_trial_element;j++)
                      {
                        ...
                        globalJacobian[csrRowIndeces_u_u[eN_i] + csrColumnOffsets_eb_u_u[ebN_i_j]] += H_s*(fluxJacobian_u_u[j]*vel_test_dS[i]+
                                                                                                           ck.ExteriorElementBoundaryDiffusionAdjointJacobian(isDOFBoundary_u[ebNE_kb],
                                                                                                                                                              ...
                                                                                                                                                              mom_uu_diff_ten_ext,
                                                                                                                                                              &vel_grad_test_dS[i*nSpace]));
                        globalJacobian[csrRowIndeces_u_v[eN_i] + csrColumnOffsets_eb_u_v[ebN_i_j]] += H_s*(fluxJacobian_u_v[j]*vel_test_dS[i]+
                        ...
                        globalJacobian[csrRowIndeces_v_u[eN_i] + csrColumnOffsets_eb_v_u[ebN_i_j]] += H_s*(fluxJacobian_v_u[j]*vel_test_dS[i]+
                                                                                                           ck.ExteriorElementBoundaryDiffusionAdjointJacobian(isDOFBoundary_u[ebNE_kb],
                                                                                                                                                              isDiffusiveFluxBoundary_v[ebNE_kb],
                                                                                                                                                              eb_adjoint_sigma,
                                                                                                                                                              vel_trial_trace_ref[ebN_local_kb_j],
                                                                                                                                                              ...
                                                                                                                                                              mom_vu_diff_ten_ext,
                                                                                                                                                              &vel_grad_test_dS[i*nSpace]));
                        globalJacobian[csrRowIndeces_v_v[eN_i] + csrColumnOffsets_eb_v_v[ebN_i_j]] += H_s*(fluxJacobian_v_v[j]*vel_test_dS[i]+
                                                                                                           ck.ExteriorElementBoundaryDiffusionAdjointJacobian(isDOFBoundary_v[ebNE_kb],
                                                                                                                                                              isDiffusiveFluxBoundary_v[ebNE_kb],
                                                                                                                                                              eb_adjoint_sigma,
                                                                                                                                                              vel_trial_trace_ref[ebN_local_kb_j],
