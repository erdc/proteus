...
namespace proteus
{
  ...
  public:
	...
	for (int i=0;i<nParticles;i++){
      ...
      mom_u_source = -phi_s_effect*porosity*g[0];// - porosity*d_mu*sigma*kappa*n[0]/(rho*(norm_n+1.0e-8));
      mom_v_source = -phi_s_effect*porosity*g[1];// - porosity*d_mu*sigma*kappa*n[1]/(rho*(norm_n+1.0e-8));
      ...
      mom_u_ham += phi_s_effect*porosity*(uStar*grad_u[0]+vStar*grad_u[1]);
      ...
      mom_v_ham += phi_s_effect*porosity*(uStar*grad_v[0]+vStar*grad_v[1]);
	...
	if (element_owned){
  	...
  	// if(phi_s<=0.0 && C==0.0)
	//   printf("dmom_u_source[0]=%f,dmom_v_source[1]=%f in updateSolidParticleTerms\n",dmom_u_source[0],dmom_v_source[1]);
      //Nitsche terms
      ...
      for(int eN=0;eN<nElements_global;eN++)
	{
	  ...
	  for(int k=0;k<nQuadraturePoints_element;k++)
		...
		// if (dmom_u_acc_u!=dmom_u_acc_u)
		// 	printf("dmom_u_acc_u in nan\n");
		// if (mom_u_adv!=mom_u_adv)
		// 	printf("mom_u_adv in nan\n");
		// if (mom_u_source!=mom_u_source)
		// 	printf("mom_u_source in nan\n");
		// if (mom_u_ham!=mom_u_ham)
		// 	printf("mom_u_ham in nan\n");
		...
		// if (dmom_v_acc_v!=dmom_v_acc_v)
		// 	printf("dmom_v_acc_v in nan\n");
		// if (mom_v_adv!=mom_v_adv)
		// 	printf("mom_v_adv in nan\n");
		// if (mom_v_source!=mom_v_source)
		// 	printf("mom_v_source in nan\n");
		// if (mom_v_ham!=mom_v_ham)
		// 	printf("mom_v_ham in nan\n");	
	  ...
	  for  (int k=0;k<nQuadraturePoints_element;k++)
	      ...
	      for(int i=0;i<nDOF_test_element;i++)
		{
		  ...
		  for(int j=0;j<nDOF_trial_element;j++) 
			...
			// if (i==j && (elementJacobian_u_u[i][j]==0. || elementJacobian_u_v[i][j]==0.))
			// {					
			// 	printf("i=j=%d, dmom_u_source[0]=%f, dmom_u_source[1]=%f, dmom_u_ham_grad_u[0]=%f,dmom_u_ham_grad_u[1]=%f\n", 
			// 	i, dmom_u_source[0], dmom_u_source[1], dmom_u_ham_grad_u[0],dmom_u_ham_grad_u[1]);
			// }
			...
			// if (i==j && (elementJacobian_v_u[i][j]==0. || elementJacobian_v_v[i][j]==0.))
			// {					
			// 	printf("i=j=%d, dmom_v_source[0]=%f, dmom_v_source[1]=%f, dmom_v_ham_grad_v[0]=%f,dmom_v_ham_grad_v[1]=%f\n", 
			// 	i, dmom_v_source[0], dmom_v_source[1], dmom_v_ham_grad_v[0],dmom_v_ham_grad_v[1]);
			// }
