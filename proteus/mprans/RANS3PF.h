...
namespace proteus
{
  ...
  public:
					...
					double* velocity)
    {
      ...
      flux_vmom = 0.0;
      flux_wmom = 0.0;
      flowDirection=porosity*(n[0]*u+n[1]*v+n[2]*w);
      speed = fabs(flowDirection);
      bc_flowDirection=porosity*(n[0]*bc_u+n[1]*bc_v+n[2]*bc_w);
		  ...
		  for(int j=0;j<nDOF_trial_element;j++) 
			...
			ck.SubgridErrorJacobian(dsubgridError_u_p[j],Lstar_u_u[i]); 
		      elementJacobian_u_u[i][j] += ck.MassJacobian_weak(dmom_u_acc_u_t,vel_trial_ref[k*nDOF_trial_element+j],vel_test_dV[i]) +
                        ck.HamiltonianJacobian_weak(dmom_u_ham_grad_u,&p_grad_trial[j_nSpace],vel_test_dV[i]) + 
			...
			ck.SubgridErrorJacobian(dsubgridError_p_u[j],Lstar_p_v[i]);
		      elementJacobian_v_v[i][j] += ck.MassJacobian_weak(dmom_v_acc_v_t,vel_trial_ref[k*nDOF_trial_element+j],vel_test_dV[i]) + 
                        ck.HamiltonianJacobian_weak(dmom_v_ham_grad_v,&p_grad_trial[j_nSpace],vel_test_dV[i]) + 
	...
	permutations[i]=i;//just to initialize
      ...
      for (int ebNI = 0; ebNI < nInteriorElementBoundaries_global; ebNI++)
		...
		right_ebN_element_kb_nDOF_test_element=(right_ebN_element*nQuadraturePoints_elementBoundary+right_kb)*nDOF_test_element;
    	      ...
    	      velocityAverage[ebN_kb_nSpace+0]=0.5*(porosity_left*u_left + porosity_right*u_right);
    	      velocityAverage[ebN_kb_nSpace+1]=0.5*(porosity_left*v_left + porosity_right*v_right);
    	      velocityAverage[ebN_kb_nSpace+2]=0.5*(porosity_left*w_left + porosity_right*w_right);
    	    }//ebNI
    	}
    }
  };//RANS3PF
  ...
  inline cppRANS3PF_base* newRANS3PF(int nSpaceIn,
				...
				int CompKernelFlag)
  {
    return proteus::chooseAndAllocateDiscretization<cppRANS3PF_base,cppRANS3PF,CompKernel>(nSpaceIn,
										   nQuadraturePoints_elementIn,
										   nDOF_mesh_trial_elementIn,
										   nDOF_trial_elementIn,
										   nDOF_test_elementIn,
