...
if ct.useHex:
    ...
    quad = True
    soname = '_'.join(
        [str(i) for i in ['rotation_qua', ct.timeIntegration_vof, ct.ncells, ct.nLevels, ct.stablization]])
else:
    soname = '_'.join(
        [str(i) for i in ['rotation_tri', ct.timeIntegration_vof, ct.ncells, ct.nLevels, ct.stablization]])
