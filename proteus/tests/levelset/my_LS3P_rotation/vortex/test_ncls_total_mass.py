#!/usr/bin/env python
"""
Test module for NCLS3P vortex with EV
"""
...
import numpy as np
import ncls_vortex_2d_test_template as ncls


class TestNCLS3PvortexEV():
    ...
    def test_ncls_T1_SSP33_ElementBasedEV(self):
        ...
        2nd order MPP edge based EV stabilization 
        These are the flags used for NCLS3P.h in this benchmark 
        ...
        ncls.vort2D.soname=ncls.vort2D.soname.replace("FE","SSP33")
        ncls.p.name = ncls.p.name.replace("FE","SSP33")
        ncls.so.name = ncls.vort2D.soname
        ...
        ncls.p.coefficients = ncls.vort2D.MyCoefficients(epsFact=ncls.vort2D.epsFactHeaviside,checkMass=ncls.vort2D.checkMass,useMetrics=ncls.vort2D.useMetrics,ME_model=0,
                                                        EDGE_VISCOSITY=1,
                                                        ENTROPY_VISCOSITY=1,
        ...
        ref_total_mass = np.loadtxt(os.path.join(run_dir,'comparison_files','total_mass_comp_0_T1_vortex_c0p1_SSP33_EdgeBasedEV_level_3_ncls3p.txt'))
