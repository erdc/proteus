...
if cDegree_ls == 0:
    ...
    if LevelModelType == RDLS.LevelModel:
        subgridError = HamiltonJacobi_ASGS_opt(
            coefficients, nd, stabFlag='2', lag=False)
    else:
        subgridError = HamiltonJacobi_ASGS(
            coefficients, nd, stabFlag='2', lag=False)
    shockCapturing = RDLS.ShockCapturing(
        coefficients, nd, shockCapturingFactor=shockCapturingFactor_rd, lag=lag_shockCapturing_rd)
