...
for i in range(3):
    for j in range(3):
        ...
        print(' '.join(['parun -v -l 5', 'ls_vortex_2d_so.py', '-C', " 'hk=%0.1f lRefinement=%d applyCorrection=False applyRedistancing=True'" %(hk[i],ref[j])]))
        os.system(' '.join(['parun -v -l 5', 'ls_vortex_2d_so.py', '-C', " 'hk=%0.1f lRefinement=%d applyCorrection=False applyRedistancing=True'" %(hk[i],ref[j])]))
