...
def symmetric2D(box=(1.0,0.4),
    ...
    for i,v in enumerate(vertexFlags):
        if v != boundaryFlags['obstacle']:
            vertexFlags[i] = 0
    for i,s in enumerate(segmentFlags):
        if s != boundaryFlags['obstacle']:
            segmentFlags[i] = 0

    v =len(vertices2)
    vertices2.append((0.0, 0.0))
