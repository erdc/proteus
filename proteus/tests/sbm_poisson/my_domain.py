...
# 2D disk domain
#===============================================================================
def get_domain_2D_disk(center, radius, he=None):
    ...
    segments = []
    segmentFlags = []
    rtheta2xy = lambda c,r,t: [c[0]+math.cos(t)*r, c[1]+math.sin(t)*r]
    vertices.append(rtheta2xy(center,radius,0.0))
    vertexFlags.append(boundaryTags['obstacle'])
    for i in range(1,N):
        vertices.append(rtheta2xy(center,radius,i*theta))
        vertexFlags.append(boundaryTags['obstacle'])
...
def get_domain_2D_square(left_bottom_cornor,L,he):
    boundaries = ['left', 'right', 'bottom', 'top', 'front', 'back', 'obstacle']
    boundaryTags = dict([(key, i + 1) for (i, key) in enumerate(boundaries)])
    vertices = [[left_bottom_cornor[0], left_bottom_cornor[1]],  #0
            [left_bottom_cornor[0]+L[0], left_bottom_cornor[1]],  #1
            [left_bottom_cornor[0]+L[0], left_bottom_cornor[1]+L[1]],  #2
            [left_bottom_cornor[0], left_bottom_cornor[1]+L[1]],  #3
            ]
    vertexFlags = [boundaryTags['bottom'],
                   boundaryTags['bottom'],
                   boundaryTags['top'],
    ...
    regions = [[left_bottom_cornor[0]+0.5*L[0], left_bottom_cornor[1]+0.5*L[1]]]
    regionFlags = [1,]
    regionConstraints=[0.5*he**2]
    domain = Domain.PlanarStraightLineGraphDomain(vertices=vertices,
                                                  vertexFlags=vertexFlags,
