...
class LevelModel(OneLevelTransport):
    ...
    def __init__(self,
                 *args,
                 **kwargs):
        OneLevelTransport.__init__(self,*args,**kwargs)
# 
...
#         
#         
        # since coefficients is already created now
        import cPoisson_M1 as c_model
        self.Poisson = c_model.cPoisson_base(self.nSpace_global,
...
#             self.getResidual(self.u[0].dof, self.globalResidualDummy)
        ...
        # other useless parameters 
        self.MOVING_DOMAIN = 1.0
        self.coefficients.useMetrics = 1.0
        self.coefficients.sc_uref = 1.0
        self.coefficients.sc_beta = 1.0
        self.uStar_dof = numpy.copy(self.u[0].dof)
        self.q[('dH_sge', 0, 0)] = numpy.zeros((self.mesh.nElements_global, self.nQuadraturePoints_element, self.nSpace_global), 'd')
        self.q['dV_last'] = -1000 * numpy.ones((self.mesh.nElements_global, self.nQuadraturePoints_element), 'd')
        self.interface_locator = numpy.zeros(self.u[0].dof.shape,'d')        
...
#                                                       self.q[('grad(w)*dV_f', 0)])
        ...
        cfemIntegrals.updateMassJacobian_weak_lowmem(dm,
                                                     ...
                                                     self.q[('w*dV_m', 0)],
                                                     elementMassMatrix)
        self.MC_a = np.zeros((self.nnz,),'d') ##global sparse pattern
        self.MC_global = SparseMat(self.nFreeDOF_global[0],
                                   self.nFreeDOF_global[0],
                                   self.nnz,
                                   self.MC_a,
        ...
        cfemIntegrals.updateGlobalJacobianFromElementJacobian_CSR(self.l2g[0]['nFreeDOF'],
                                                                  ...
                                                                  self.csrColumnOffsets[(0, 0)],
                                                                  elementMassMatrix,
                                                                  self.MC_global)##only assemble 0-0 part
        for i in range(self.nFreeDOF_global[0]):
            dof_i_global = self.offset[0]+i*self.stride[0] ##only 1st component
            _ML[i] = self.MC_a[self.rowptr[dof_i_global]:self.rowptr[dof_i_global + 1]].sum()

#     def getResidual(self, u, r):
#         r.fill(0.0)
    def update_solution_ale(self, u, r): #####because of moving mesh and making no changes
        if self.coefficients.ssp_order == 1:
            self.get_ssp1_ale(u, r)
        elif self.coefficients.ssp_order == 2:
            self.get_ssp2_ale(u, r)
        elif self.coefficients.ssp_order == 3:
            self.get_ssp3_ale(u, r)
            
    def get_ssp1_ale(self, u, r):
        #update solution
        self.euler_step(u, r)
        #update mesh
        self.mesh.nodeArray[:] += self.timeIntegration.dt * self.mesh.nodeVelocityArray

    def update_ssp_step(self, u_new, node_new, ML_new, u_old, node_old, ML_old, a, b):
        node_new *= a
        node_new += b*node_old
        
        u_new *= a*ML_new
        u_new += b*u_old*ML_old
    ...
    def get_ssp2_ale(self, u, r):##update self.mesh.nodeArray, r, self.coefficients.ML_new
        
        node_array_old = np.copy(self.mesh.nodeArray)
#         velocity_node_array_old = np.copy(self.mesh.nodeVelocityArray)
        self.coefficients.ML_old[:] = self.coefficients.ML_new
        
        #update solution
        r1 = np.zeros_like(r)
        self.euler_step(u, r1)
        #update mesh
        self.mesh.nodeArray[:] += self.timeIntegration.dt * self.mesh.nodeVelocityArray
        #update solution
        self.euler_step(r1, r)
        #update mesh
        self.mesh.nodeArray[:] += self.timeIntegration.dt * self.mesh.nodeVelocityArray
        #modify the mesh
#         self.mesh.nodeArray *= 0.5 
#         self.mesh.nodeArray += 0.5*node_array_old 
#         r *= 0.5*self.coefficients.ML_new
#         r += 0.5*self.coefficients.ML_old*u
#          
#         self.coefficients.ML_new *= 0.5
#         self.coefficients.ML_new += 0.5*self.coefficients.ML_old
#          
#         r /= self.coefficients.ML_new
        
        self.update_ssp_step(r,self.mesh.nodeArray,self.coefficients.ML_new,
                             u,node_array_old,self.coefficients.ML_old,
                             0.5, 0.5)
        
    def get_ssp3_ale(self, u, r):
        #previosu data
        node_array_old = np.copy(self.mesh.nodeArray)
#         velocity_node_array_old = np.copy(self.mesh.nodeVelocityArray)
        self.coefficients.ML_old[:] = self.coefficients.ML_new
        ...
        self.mesh.nodeArray[:] += self.timeIntegration.dt * self.mesh.nodeVelocityArray
        #update solution
        self.euler_step(r1, r2)
        #update mesh
        self.mesh.nodeArray[:] += self.timeIntegration.dt * self.mesh.nodeVelocityArray
        #update ssp step
        self.update_ssp_step(r2,self.mesh.nodeArray,self.coefficients.ML_new,
                             u,node_array_old,self.coefficients.ML_old,
                             0.25, 0.75)
        
        #update solution
        self.euler_step(r2, r)
        #update mesh
        self.mesh.nodeArray[:] += self.timeIntegration.dt * self.mesh.nodeVelocityArray
        #update ssp step
        self.update_ssp_step(r,self.mesh.nodeArray,self.coefficients.ML_new,
...
#         self.timeIntegration.calculateU(u)
        ...
        self.coefficients.apply_dirichlet_bc(self.u[0].dof)######YY-Q:after or before setUnknowns????
...
#         self.ncls.calculateResidual_entropy_viscosity(
            ...
            self.u[0].dof,###########since self.setUnknowns, u is put into self.u[0].dof
            r,  #######this is the only output
            self.offset[0], #0
            self.stride[0], #1
            ...
            self.csrRowIndeces[(0, 0)],  # row indices (convenient for element loops) # is for 0-component 
            self.csrColumnOffsets[(0, 0)],  # column indices (convenient for element loops)# is for 0-component # is a global index
            self.csrColumnOffsets_eb[(0, 0)],  # indices for boundary terms
            self.quantDOFs,
            self.coefficients.ML_new, ########it is used and updated at the same place
            self.isActiveDOF,
...
#         pass
    ...
    def getJacobian(self, jacobian):
        ...
        self.Poisson.calculateJacobian(  # element #FOR SUPG
            self.timeIntegration.dt,
            self.u[0].femSpace.elementMaps.psi,
            self.u[0].femSpace.elementMaps.grad_psi,
            self.mesh.nodeArray,
