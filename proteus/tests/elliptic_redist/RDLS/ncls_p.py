...
class init_cond(object):
    ...
    def uOfXT(self,x,t):
        import numpy as np
        return self.scaling*(self.radius - math.sqrt((x[0]-self.xc)**2 + (x[1]-self.yc)**2))**self.r
...
analyticalSolution = {0:init_cond(L,scaling=1.0,r=3)}
initialConditions  = {0:init_cond(L,scaling=1.5)}
