...
class TestAddedMass2DIB(unittest.TestCase):
    ...
    def test_AddedMass_2DIB(self):
        from proteus import defaults
        from . import addedmass2DIB as am2D
        am2D.myTpFlowProblem.initializeAll()
        so = am2D.myTpFlowProblem.so
        so.name = 'addedMass2DIB'
        pList = []
        nList = []
        ...
        #np.savetxt('Aij_sol2DIB.csv', Aij, delimiter=',')
        Aij_sol = np.genfromtxt(os.path.join(modulepath, 'Aij_sol2DIB.csv'), delimiter=',')
