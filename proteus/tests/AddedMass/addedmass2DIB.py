...
cyl = st.Circle(domain, radius=0.5, coords=[tank_dim[0]/2., tank_dim[1]/2.], barycenter=[tank_dim[0]/2., tank_dim[1]/2.], nPoints=int(math.ceil(2*math.pi*0.5/he)))
cyl.setHoles(holes=np.array([cyl.coords]))
...
for key in cyl.boundaryTags_global:
    flags_rigidbody[cyl.boundaryTags_global[key]] = 1.
...
for s in system.subcomponents:
    if type(s) is fsi.ProtChBody:
        for i in range(s.i_start, s.i_end):
            flags_rigidbody[i] = 1
#flags_rigidbody = np.array([6], dtype='int32')
m.addedMass.p.CoefficientsOptions.flags_rigidbody = flags_rigidbody
print("flags_rigidbody ", flags_rigidbody)
