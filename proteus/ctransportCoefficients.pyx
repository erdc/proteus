...
def constantVelocityLevelSetEvaluate(np.ndarray b,
                      ...
                      np.ndarray H,
                      np.ndarray dH):
    cconstantVelocityLevelSetEvaluate(x.size,
                      f.shape[-1],
                      <double*> b.data,
                      <double*> x.data,
                      <double*> u.data,
...
def constantNormalVelocityLevelSetEvaluate(double b,
                        ...
                        np.ndarray H,
                        np.ndarray dH):
    cconstantNormalVelocityLevelSetEvaluate(x.size,
                        f.shape[-1],
                        b,
                        <double*> x.data,
                        <double*> u.data,
...
def unitSquareVortexLevelSetEvaluate(double t,
                      ...
                      np.ndarray H,
                      np.ndarray dH):
    cunitSquareVortexLevelSetEvaluate(x.size,
                      f.shape[-1],
                      t,
                      <double*> x.data,
                      <double*> u.data,
...
def unitSquareRotationLevelSetEvaluate(double t,
                    ...
                    np.ndarray H,
                    np.ndarray dH):
    cunitSquareRotationLevelSetEvaluate(x.size,
                    f.shape[-1],
                     t,
                    <double*> x.data,
                    <double*> u.data,
...
def conservativeHeadRichardsL2projMualemVanGenuchtenHetEvaluate(const double rho,
                                 ...
                                 np.ndarray a,
                                 np.ndarray da):
    cconservativeHeadRichardsL2projMualemVanGenuchtenHetEvaluate(f.size/f.shape[-1]/f.shape[-2],
                                 f.shape[-2],
                                 f.shape[-1],
                                 rho,
                                 <double*> gravity.data,
...
def conservativeHeadRichardsMualemVanGenuchten_sd_het(np.ndarray  rowptr,
    ...
    if linearize_at_zero == 1:
        cconservativeHeadRichardsMualemVanGenuchten_sd_het_linearized_at_saturation(f.size/f.shape[-2]/f.shape[-1],
                               f.shape[-2],
                               f.shape[-1],
                                pc_eps,
                               <int*>  rowptr.data,
        ...
        cconservativeHeadRichardsMualemVanGenuchten_sd_het(f.size/f.shape[-2]/f.shape[-1],
                               f.shape[-2],
                               f.shape[-1],
                                pc_eps,
                               <int*>  rowptr.data,
...
def conservativeHeadRichardsMualemVanGenuchtenHetEvaluateV2(np.ndarray materialTypes,
                                 ...
                                 np.ndarray a,
                                 np.ndarray da):
    cconservativeHeadRichardsMualemVanGenuchtenHetEvaluateV2(f.size/f.shape[-1]/f.shape[-2],
                                 f.shape[-2],
                                 f.shape[-1],
                                 <int*> materialTypes.data,
                                 rho,
...
def seepageBrezis(np.ndarray materialTypes,
           ...
           np.ndarray a,
           np.ndarray da):
    cseepageBrezis(f.size/f.shape[-1]/f.shape[-2],
            f.shape[-2],
            f.shape[-1],
            <int*> materialTypes.data,
            epsFact,
...
def conservativeHeadRichardsMualemVanGenuchtenHetEvaluate(const double rho,
                                                           ...
                                                           np.ndarray a,
                                                           np.ndarray da):
    cconservativeHeadRichardsMualemVanGenuchtenHetEvaluate(f.size/f.shape[-1],
                                                           f.shape[-1],
                                                           rho,
                                                           <double*> gravity.data,
                                                           <double*> alpha.data,
...
def conservativeSatRichardsMualemVanGenuchtenHomEvaluate(const double rho,
                                                          ...
                                                          np.ndarray phi,
                                                          np.ndarray dphi):
    cconservativeSatRichardsMualemVanGenuchtenHomEvaluate(f.size/f.shape[-1],
                                                        f.shape[-1],
                                                        rho,
                                                        <double*> gravity.data,
                                                        <double*> x.data,
...
def conservativeHeadRichardsBrooksCoreyBurdineHetEvaluate(const double rho,
                                                           ...
                                                           np.ndarray a,
                                                           np.ndarray da):
    cconservativeHeadRichardsBrooksCoreyBurdineHetEvaluate(f.size/f.shape[-1],
                                                        f.shape[-1],
                                                        rho,
                                                        <double*> gravity.data,
                                                        <double*> Lambda.data,
...
def conservativeHeadRichardsBrooksCoreyBurdineHomEvaluate(const double rho,
                                                        ...
                                                        np.ndarray a,
                                                        np.ndarray da):
    cconservativeHeadRichardsBrooksCoreyBurdineHomEvaluate(f.size/f.shape[-1],
                                                        f.shape[-1],
                                                        rho,
                                                        beta,
                                                        <double*> gravity.data,
...
def conservativeHeadRichardsBCBfromMVGHomEvaluate(const double rho,
                           ...
                           np.ndarray a,
                           np.ndarray da):
    cconservativeHeadRichardsBCBfromMVGHomEvaluate(f.size/f.shape[-1],
                                                f.shape[-1],
                                                rho,
                                                <double*> gravity.data,
                                                alpha,
...
def conservativeHeadRichardsJLeverettEvaluate(np.ndarray materialTypes,
                                       ...
                                       np.ndarray a,
                                       np.ndarray da):
    cconservativeHeadRichardsJLeverett(f.size/f.shape[-1]/f.shape[-2],
                                    f.shape[-2],
                                    f.shape[-1],
                                    <int*> materialTypes.data,
                                    rho,
...
def conservativeHeadRichardsJLeverettAniEvaluate(np.ndarray materialTypes,
                                       ...
                                       np.ndarray a,
                                       np.ndarray da):
    cconservativeHeadRichardsJLeverettAni(f.size/f.shape[-1]/f.shape[-2],
                                    f.shape[-2],
                                    f.shape[-1],
                                    <int*> materialTypes.data,
                                    rho,
