...
def unitSquareVortexEvaluate(double t,
                            ...
                            np.ndarray f,
                            np.ndarray df):
    cunitSquareVortexEvaluate(x.size/x.shape[x.ndim-1],
                            f.shape[f.ndim-1],
                            t,
                            <double*> x.data,
                            <double*> u.data,
...
def conservativeHeadRichardsMualemVanGenuchtenHomEvaluate(const double rho,
                                                           ...
                                                           np.ndarray phi,
                                                           np.ndarray dphi):
    cconservativeHeadRichardsMualemVanGenuchtenHomEvaluate(f.size/f.shape[f.ndim-1],
                                                           f.shape[f.ndim-1],
                                                           rho,
                                                           beta,
                                                           <double*> gravity.data,
...
def conservativeHeadRichardsL2projMualemVanGenuchtenHetEvaluate(const double rho,
                                 ...
                                 np.ndarray a,
                                 np.ndarray da):
    cconservativeHeadRichardsL2projMualemVanGenuchtenHetEvaluate(f.size/f.shape[f.ndim-1]/f.shape[-2],
                                 f.shape[-2],
                                 f.shape[f.ndim-1],
                                 rho,
                                 <double*> gravity.data,
...
def conservativeTotalHeadRichardsMualemVanGenuchtenHomEvaluate(const double rho,
                                ...
                                np.ndarray phi,
                                np.ndarray dphi):
    cconservativeTotalHeadRichardsMualemVanGenuchtenHomEvaluate(f.size/f.shape[f.ndim-1],
                                f.shape[f.ndim-1],
                                rho,
                                <double*> gravity.data,
                                <double*> x.data,
...
def conservativeHeadRichardsMualemVanGenuchten_sd_het(np.ndarray  rowptr,
    ...
    if linearize_at_zero == 1:
        cconservativeHeadRichardsMualemVanGenuchten_sd_het_linearized_at_saturation(f.size/f.shape[-2]/f.shape[f.ndim-1],
                               f.shape[-2],
                               f.shape[f.ndim-1],
                                pc_eps,
                               <int*>  rowptr.data,
        ...
        cconservativeHeadRichardsMualemVanGenuchten_sd_het(f.size/f.shape[-2]/f.shape[f.ndim-1],
                               f.shape[-2],
                               f.shape[f.ndim-1],
                                pc_eps,
                               <int*>  rowptr.data,
...
def conservativeHeadRichardsMualemVanGenuchtenHetEvaluateV2(np.ndarray materialTypes,
                                 ...
                                 np.ndarray a,
                                 np.ndarray da):
    cconservativeHeadRichardsMualemVanGenuchtenHetEvaluateV2(f.size/f.shape[f.ndim-1]/f.shape[-2],
                                 f.shape[-2],
                                 f.shape[f.ndim-1],
                                 <int*> materialTypes.data,
                                 rho,
...
def seepageBrezis(np.ndarray materialTypes,
           ...
           np.ndarray a,
           np.ndarray da):
    cseepageBrezis(f.size/f.shape[f.ndim-1]/f.shape[-2],
            f.shape[-2],
            f.shape[f.ndim-1],
            <int*> materialTypes.data,
            epsFact,
...
def conservativeHeadRichardsMualemVanGenuchtenHetEvaluate(const double rho,
                                                           ...
                                                           np.ndarray a,
                                                           np.ndarray da):
    cconservativeHeadRichardsMualemVanGenuchtenHetEvaluate(f.size/f.shape[f.ndim-1],
                                                           f.shape[f.ndim-1],
                                                           rho,
                                                           <double*> gravity.data,
                                                           <double*> alpha.data,
...
def conservativeSatRichardsMualemVanGenuchtenHomEvaluate(const double rho,
                                                          ...
                                                          np.ndarray phi,
                                                          np.ndarray dphi):
    cconservativeSatRichardsMualemVanGenuchtenHomEvaluate(f.size/f.shape[f.ndim-1],
                                                        f.shape[f.ndim-1],
                                                        rho,
                                                        <double*> gravity.data,
                                                        <double*> x.data,
...
def conservativeHeadRichardsBrooksCoreyBurdineHetEvaluate(const double rho,
                                                           ...
                                                           np.ndarray a,
                                                           np.ndarray da):
    cconservativeHeadRichardsBrooksCoreyBurdineHetEvaluate(f.size/f.shape[f.ndim-1],
                                                        f.shape[f.ndim-1],
                                                        rho,
                                                        <double*> gravity.data,
                                                        <double*> Lambda.data,
...
def conservativeHeadRichardsBrooksCoreyBurdineHomEvaluate(const double rho,
                                                        ...
                                                        np.ndarray a,
                                                        np.ndarray da):
    cconservativeHeadRichardsBrooksCoreyBurdineHomEvaluate(f.size/f.shape[f.ndim-1],
                                                        f.shape[f.ndim-1],
                                                        rho,
                                                        beta,
                                                        <double*> gravity.data,
...
def conservativeHeadRichardsBCBfromMVGHomEvaluate(const double rho,
                           ...
                           np.ndarray a,
                           np.ndarray da):
    cconservativeHeadRichardsBCBfromMVGHomEvaluate(f.size/f.shape[f.ndim-1],
                                                f.shape[f.ndim-1],
                                                rho,
                                                <double*> gravity.data,
                                                alpha,
...
def conservativeHeadRichardsMualemVanGenuchtenHetEvaluateV2withUpwind(const int upwindFlag,
    ...
    cconservativeHeadRichardsMualemVanGenuchtenHetEvaluateV2withUpwind(upwindFlag,
                                    computeAverages,
                                    f.size/f.shape[f.ndim-1]/f.shape[-2],
                                    f.shape[-2],
                                    f.shape[f.ndim-1],
...
def conservativeHeadRichardsJLeverettEvaluate(np.ndarray materialTypes,
                                       ...
                                       np.ndarray a,
                                       np.ndarray da):
    cconservativeHeadRichardsJLeverett(f.size/f.shape[f.ndim-1]/f.shape[-2],
                                    f.shape[-2],
                                    f.shape[f.ndim-1],
                                    <int*> materialTypes.data,
                                    rho,
...
def conservativeHeadRichardsJLeverettAniEvaluate(np.ndarray materialTypes,
                                       ...
                                       np.ndarray a,
                                       np.ndarray da):
    cconservativeHeadRichardsJLeverettAni(f.size/f.shape[f.ndim-1]/f.shape[-2],
                                    f.shape[-2],
                                    f.shape[f.ndim-1],
                                    <int*> materialTypes.data,
                                    rho,
