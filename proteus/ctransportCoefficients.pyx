...
def linearADR_ConstantCoefficientsEvaluate(const double M,
                                            ...
                                            np.ndarray r,
                                            np.ndarray dr):
    clinearADR_ConstantCoefficientsEvaluate(x.size/x.shape[-1],
                                            f.shape[-1],
                                            M,
                                            <double*> A.data,
                                            <double*> B.data,
...
def groundwaterTransportCoefficientsEvaluate(const double omega,
                                              ...
                                              np.ndarray df,
                                              np.ndarray a):
    cgroundwaterTransportCoefficientsEvaluate(f.size/f.shape[-1],
                                            f.shape[-1],
                                            omega,
                                            d,
                                            alpha_L,
...
def groundwaterBiodegradation01EvaluateFC(const double omega,
                       ...
                       np.ndarray dr_x_de,
                       np.ndarray dr_x_dx):
    cgroundwaterBiodegradation01EvaluateFC(f_c.size/f_c.shape[-1],
                       f_c.shape[-1],
                       omega,
                       d_c,
                       d_e,
...
def groundwaterBryantDawsonIonExEvaluateFC(const double omega,
                        ...
                        np.ndarray dr_h_dm,
                        np.ndarray dr_h_dh):
    cgroundwaterBryantDawsonIonExEvaluateFC(f_m.size/f_m.shape[-1],
                        f_m.shape[-1],
                        omega,
                        d_m,
                        d_h,
...
def nonlinearADR_pqrstEvaluate(const double M,
                                ...
                                np.ndarray r,
                                np.ndarray dr):
    cnonlinearADR_pqrstEvaluate(x.size/x.shape[-1],
                                f.shape[-1],
                                M,
                                <double*> A.data,
                                <double*> B.data,
...
def unitSquareRotationEvaluate(np.ndarray x,
                                ...
                                np.ndarray f,
                                np.ndarray df):
    cunitSquareRotationEvaluate(x.size/x.shape[-1],
                                f.shape[-1],
                                <double*> x.data,
                                <double*> u.data,
                                <double*> m.data,
...
def unitCubeRotationEvaluate(np.ndarray x,
                    ...
                    np.ndarray f,
                    np.ndarray df):
    cunitCubeRotationEvaluate(x.size/x.shape[-1],
                    f.shape[-1],
                    <double*> x.data,
                    <double*> u.data,
                    <double*> m.data,
...
def unitSquareVortexEvaluate(double t,
                            ...
                            np.ndarray f,
                            np.ndarray df):
    cunitSquareVortexEvaluate(x.size/x.shape[-1],
                            f.shape[-1],
                            t,
                            <double*> x.data,
                            <double*> u.data,
...
def conservativeHeadRichardsMualemVanGenuchtenHomEvaluate(const double rho,
                                                           ...
                                                           np.ndarray phi,
                                                           np.ndarray dphi):
    cconservativeHeadRichardsMualemVanGenuchtenHomEvaluate(f.size/f.shape[-1],
                                                           f.shape[-1],
                                                           rho,
                                                           beta,
                                                           <double*> gravity.data,
...
def conservativeTotalHeadRichardsMualemVanGenuchtenHomEvaluate(const double rho,
                                ...
                                np.ndarray phi,
                                np.ndarray dphi):
    cconservativeTotalHeadRichardsMualemVanGenuchtenHomEvaluate(f.size/f.shape[-1],
                                f.shape[-1],
                                rho,
                                <double*> gravity.data,
                                <double*> x.data,
